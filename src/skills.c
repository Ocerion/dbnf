...
*/
  ...
  if( IS_NPC( ch ) && ch->master != NULL && skill_table[sn]->race_level[ch->race] == 0 )
    return TRUE;
  ...
  if( !IS_NPC( ch ) && skill_table[sn]->skill_level[ch->class] > ch->exp && ch->pcdata->learned[sn] > 0 )
  {
    send_to_char( "You have become too weak too use that ability.\n\r", ch );
    return TRUE;
  }
...
*/
    ...
    if( ( number_percent(  ) + skill_table[sn]->difficulty * 5 ) > ( IS_NPC( ch ) ? 75 : LEARNED( ch, sn ) ) )
    {
      ...
      if( mana && !str_cmp( spell_name( skill_table[sn]->spell_fun ), "smaug_spell" ) )
      {
        ch->mana -= mana / 2;
      }
      return TRUE;
  ...
  if( !IS_NPC( ch ) && is_splitformed( ch ) )
  {
    ...
    for( och = ch->in_room->first_person; och; och = och_next )
    {
      ...
      if( IS_NPC( och ) && is_splitformed( och ) && och->master == ch )
      {
        ( *skill_table[sn]->skill_fun ) ( och, argument );
      }
    }
  }
  end_timer( &time_used );
...
*/
  ...
  if( !IS_NPC( ch ) && skill_table[sn]->skill_level[ch->class] > ch->exp && ch->pcdata->learned[sn] > 0 )
  {
    send_to_char( "You have become to weak to use that ability.\n\r", ch );
    return TRUE;
  }
...
void do_slookup( CHAR_DATA * ch, char *argument )
{
  ...
  if( !str_cmp( arg, "all" ) )
  {
    ...
    for( sn = 0; sn < top_sn && skill_table[sn] && skill_table[sn]->name; sn++ )
      pager_printf( ch, "&cSn: &w%4d &cSlot: &w%4d &cSkill/spell: '&w%-20s&c' Damtype: &w%s\n\r",
  ...
  else
  {
    ...
    ch_printf( ch, "\n\r" );
    ch_printf( ch, "&cSn: &w%4d &cSlot: &w%4d &c%s: '&w%-20s&c'\n\r",
               sn, skill->slot, skill_tname[skill->type], skill->name );
    if( skill->info )
      ch_printf( ch, "&cDamType: &w%s  &cActType: &w%s   &cClassType: &w%s   &cPowerType: &w%s\n\r",
                 spell_damage[SPELL_DAMAGE( skill )],
                 spell_action[SPELL_ACTION( skill )], spell_class[SPELL_CLASS( skill )], spell_power[SPELL_POWER( skill )] );
    if( skill->flags )
    {
      int x;
    ...
    ch_printf( ch, "&cSaves: &w%s  &cSaveEffect: &w%s\n\r",
               spell_saves[( int )skill->saves], spell_save_effect[SPELL_SAVE( skill )] );

    if( skill->difficulty != '\0' )
      ch_printf( ch, "&cDifficulty: &w%d\n\r", ( int )skill->difficulty );

    ch_printf( ch, "&cType: &w%s  &cTarget: &w%s  &cMinpos: &w%d  &cBeats: &w%d  &cRange: &w%d\n\r",
               skill_tname[skill->type],
               target_type[URANGE( TAR_IGNORE, skill->target, TAR_OBJ_INV )],
               skill->minimum_position, skill->beats, skill->range );
    ch_printf( ch, "&cFlags: &w%d  &cGuild: &w%d  &cValue: &w%d  &cInfo: &w%d  &cCode: &w%s\n\r",
               skill->flags,
               skill->guild,
               skill->value,
               skill->info, skill->skill_fun ? skill_name( skill->skill_fun ) : spell_name( skill->spell_fun ) );
    ch_printf( ch, "&cMana: &w%d  &cFocus: &w%d  &cCanCharge: &w%d\n\r", skill->min_mana, skill->focus, skill->canCharge );
    ch_printf( ch, "&cSectors Allowed: &w%s\n",
               skill->spell_sector ? flag_string( skill->spell_sector, sec_flags ) : "All" );
    ch_printf( ch, "&cDammsg: &w%s\n\r&cWearoff: &w%s\n",
               skill->noun_damage, skill->msg_off ? skill->msg_off : "(none set)" );
    if( skill->dice && skill->dice[0] != '\0' )
      ch_printf( ch, "&cDice: &w%s\n\r", skill->dice );
    if( skill->teachers && skill->teachers[0] != '\0' )
      ch_printf( ch, "&cTeachers: &w%s\n\r", skill->teachers );
    if( skill->components && skill->components[0] != '\0' )
      ch_printf( ch, "&cComponents: &w%s\n\r", skill->components );
    if( skill->participants )
      ch_printf( ch, "&cParticipants: &w%d\n\r", ( int )skill->participants );
    ...
    for( aff = skill->affects; aff; aff = aff->next )
    {
      if( aff == skill->affects )
        send_to_char( "\n\r", ch );
      sprintf( buf, "&cAffect &w%d", ++cnt );
      if( aff->location )
      {
        strcat( buf, " &cmodifies&w " );
    ...
    if( skill->hit_char && skill->hit_char[0] != '\0' )
      ch_printf( ch, "&cHitchar   : &w%s\n\r", skill->hit_char );
    if( skill->hit_vict && skill->hit_vict[0] != '\0' )
      ch_printf( ch, "&cHitvict   : &w%s\n\r", skill->hit_vict );
    if( skill->hit_room && skill->hit_room[0] != '\0' )
      ch_printf( ch, "&cHitroom   : &w%s\n\r", skill->hit_room );
    if( skill->hit_dest && skill->hit_dest[0] != '\0' )
      ch_printf( ch, "&cHitdest   : &w%s\n\r", skill->hit_dest );
    if( skill->miss_char && skill->miss_char[0] != '\0' )
      ch_printf( ch, "&cMisschar  : &w%s\n\r", skill->miss_char );
    if( skill->miss_vict && skill->miss_vict[0] != '\0' )
      ch_printf( ch, "&cMissvict  : &w%s\n\r", skill->miss_vict );
    if( skill->miss_room && skill->miss_room[0] != '\0' )
      ch_printf( ch, "&cMissroom  : &w%s\n\r", skill->miss_room );
    if( skill->die_char && skill->die_char[0] != '\0' )
      ch_printf( ch, "&cDiechar   : &w%s\n\r", skill->die_char );
    if( skill->die_vict && skill->die_vict[0] != '\0' )
      ch_printf( ch, "&cDievict   : &w%s\n\r", skill->die_vict );
    if( skill->die_room && skill->die_room[0] != '\0' )
      ch_printf( ch, "&cDieroom   : &w%s\n\r", skill->die_room );
    if( skill->imm_char && skill->imm_char[0] != '\0' )
      ch_printf( ch, "&cImmchar   : &w%s\n\r", skill->imm_char );
    if( skill->imm_vict && skill->imm_vict[0] != '\0' )
      ch_printf( ch, "&cImmvict   : &w%s\n\r", skill->imm_vict );
    if( skill->imm_room && skill->imm_room[0] != '\0' )
      ch_printf( ch, "&cImmroom   : &w%s\n\r", skill->imm_room );
    ch_printf( ch, "\n\r" );
    if( skill->type != SKILL_HERB )
    {
      if( skill->type != SKILL_RACIAL )
      {
        send_to_char( "&c--------------------------[&wCLASS USE&c]--------------------------&w\n\r", ch );
        for( iClass = 0; iClass < MAX_PC_CLASS; iClass++ )
        {
           ...
           */
          /*
           * sprintf(buf+3, "&c(&R%d&c) max: &w%3d%%  &cpl req: &w%s",
           */
          sprintf( buf, "%s&c(&R%d&c) max: &w%3d%%  &cpl req: &w%s",
                   class_table[iClass]->who_name,
                   iClass, skill->skill_adept[iClass], num_punct_ld( skill->skill_level[iClass] ) );
          /*
           * if ( iClass % 2 == 2 )
           * strcat(buf, "\n\r" );
           * else
      ...
      else
      {
        ...
        {
          sprintf( buf, "&w%8.8s)&c pl req: &w%3d &cmax: &w%2d%%",
...
void do_sset( CHAR_DATA * ch, char *argument )
{
  ...
  if( arg1[0] == '\0' || arg2[0] == '\0' || argument[0] == '\0' )
  {
    ...
    {
      send_to_char( "&wor:     sset save skill table\n\r", ch );
      send_to_char( "&wor:     sset save herb table\n\r", ch );
      send_to_char( "&wor:     sset create skill 'new skill'\n\r", ch );
      send_to_char( "&wor:     sset create herb 'new herb'\n\r", ch );
      send_to_char( "&wor:     sset create ability 'new ability'\n\r", ch );
    }
    if( get_trust( ch ) > LEVEL_GREATER )
    {
      send_to_char( "&wor:     sset <sn>     <field> <value>\n\r", ch );
      send_to_char( "&wor:     &Rsset <sn> pl <race#> <value>\n\r", ch );
      send_to_char( "&wor:     &Rsset <sn> max <race#> <value>\n\r", ch );
      send_to_char( "&w\n\rField being one of:\n\r", ch );
      send_to_char( "&w  name code target minpos slot mana beats dammsg wearoff guild minlevel\n\r", ch );
      send_to_char( "&w  type damtype acttype classtype powertype seffect flag dice value difficulty\n\r", ch );
      send_to_char( "&w  affect rmaffect &cpl&w /*level adept */hit miss die imm (char/vict/room)\n\r", ch );
      send_to_char( "&w  components teachers /*racelevel raceadept */focus cancharge\n\r", ch );
      send_to_char( "&w  sector &cmax&w \n\r", ch );
      send_to_char( "&wAffect having the fields: <location> <modfifier> [duration] [bitvector]\n\r", ch );
      send_to_char( "&w(See AFFECTTYPES for location, and AFFECTED_BY for bitvector)\n\r", ch );
    }
    send_to_char( "&wSkill being any skill or spell.\n\r", ch );
    return;
  }

  if( get_trust( ch ) > LEVEL_GREATER && !str_cmp( arg1, "save" ) && !str_cmp( argument, "table" ) )
  {
...
*/
    ...
    {
      skill->min_mana = URANGE( 0, atoi( argument ), 99999 );
      send_to_char( "Ok.\n\r", ch );
      return;
    }
    if( !str_cmp( arg2, "focus" ) )
    {
      skill->focus = URANGE( 0, atoi( argument ), 1000 );
      send_to_char( "Ok.\n\r", ch );
      return;
    }
    if( !str_cmp( arg2, "cancharge" ) )
    {
      skill->canCharge = URANGE( 0, atoi( argument ), 100 );
      send_to_char( "Ok.\n\r", ch );
      return;
    }
...
/*	    char bitvector[MAX_INPUT_LENGTH];	*/
    ...
    if( !str_cmp( arg2, "pl" ) )
    {
      ...
      class = atoi( arg3 );
      race = atoi( arg3 );
      if( class >= MAX_PC_CLASS || class < 0 || race >= MAX_PC_RACE || race < 0 )
      {
        send_to_char( "Not a valid class or race.\n\r", ch );
      }
      else
      {
        v = atof( argument );
        rv = atoi( argument );
        skill->skill_level[class] = v;
        skill->race_level[race] = URANGE( 0, rv, MAX_LEVEL );
      }
      return;
    }
    /*
  ...
  if( ch->level < LEVEL_GREATER && !IS_NPC( victim ) && ch != victim )
  {
    send_to_char( "You can not sset players.\n\r", ch );
    return;
  }
  ...
  if( fAll )
  {
    for( sn = 0; sn < top_sn; sn++ )
    {
       ...
       */
      /*
       * Narn is dumb, ch changed to victim by Saiyr 
       */
      if( skill_table[sn]->min_level[victim->class] == 0 || skill_table[sn]->skill_level[victim->class] == 0 )
        continue;

      if( skill_table[sn]->name && ( victim->exp >= skill_table[sn]->skill_level[victim->class] || value == 0 ) )
      {
...
{
  int adept, sklvl, percent, chance;
  double learn;
  ...
  if( ch->pcdata->learned[sn] < adept )
  {
    ...
    if( percent >= chance )
      /* originally 175–225 → now ~106–136 */
      learn = (double)get_curr_int( ch ) / number_range( 106, 136 );
    else
      /* originally 275–325 → now ~167–197 */
      learn = (double)get_curr_int( ch ) / number_range( 167, 197 );
...
*/
  ...
  if( !can_drop_obj( ch, obj )
      || IS_OBJ_STAT( obj, ITEM_INVENTORY ) || IS_OBJ_STAT( obj, ITEM_PROTOTYPE ) || obj->level > ch->exp )
  {
...
void do_bite( CHAR_DATA * ch, char *argument )
{
  ...
  if( can_use_skill( ch, number_percent(  ), gsn_bite ) )
  {
    ...
    global_retcode = damage( ch, victim, number_range( 1, ch->level ), TYPE_HIT );
  }
  else
  {
...
//      gain_exp( ch, 0 - lose );
    ...
    if( ch->desc )
      lose /= 2;
//  gain_exp( ch, 0 - lose );
      ch_printf( ch, "You can only fail!  You lose %d exps.\n\r", lose );
      return;
  }
...
void do_poison_weapon( CHAR_DATA * ch, char *argument )
{
   ...
   */
  obj->timer = UMIN( obj->level, ch->exp );
  ...
  else if( !str_prefix( arg, "berserk" ) )
  {
    ...
    if( number_percent(  ) < LEARNED( ch, gsn_style_berserk ) )
    {
      ...
      if( ch->fighting )
      {
        ...
        if( IS_PKILL( ch ) )
          act( AT_ACTION, "$n moves into a berserk posture.", ch, NULL, NULL, TO_ROOM );
      }
