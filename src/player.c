...
void do_score( CHAR_DATA * ch, char *argument )
{
  ...
  if( get_curr_int( sch ) >= 100 || sch->perm_int >= 100 )
    pager_printf( ch, "INT  : &W%-3d&C(&w%3d.%2.2d&C)  eDEF: &R%-2d&C              CurrTime: &w%s\r&C",
                  get_curr_int( sch ), sch->perm_int, sch->pcdata->tInt, get_conDef( sch ), ctime( &current_time ) );
  else
  ...
  if( is_android_e( sch ) && sch->pcdata->learned[gsn_battery] > 0 )
    pager_printf_color( ch, "                                                   Stored Ki Energy: [&R%3.3d&C]\n\r",
                        ...
                        if (is_saiyan(sch)) {
                          pager_printf(ch, "RAGE : &R%4.4d&C   (Accumulates through battle and emotion)\n\r", sch->rage);
                      }
...
pager_printf(ch, "RAGE : &R%-6d&C   (Accumulates through damage and stress in battle)\n\r", ch->rage);
