...
{
    return ch->race == 4; // Replace with your actual Reploid race ID or logic
}
...
const int transformation_flags[] = {
  AFF_SSJ, AFF_SSJ2, AFF_SSJ3, AFF_SSJ4, AFF_SSJGOD, AFF_LSSJ, AFF_HSSJ, AFF_ESSJ,
  AFF_USSJ, AFF_USSJ2, AFF_KAIOKEN, AFF_HYPER, AFF_EXTREME,
  AFF_SNAMEK, AFF_UNAMEK, AFF_MICER, AFF_MYSTIC,
  AFF_ICER2, AFF_ICER3, AFF_ICER4, AFF_ICER5,
  AFF_SEMIPERFECT, AFF_PERFECT, AFF_ULTRAPERFECT,
  AFF_GROWTH, AFF_GIANT,
  AFF_EVIL_TRANS, AFF_SUPER_TRANS, AFF_KID_TRANS,
  AFF_MAJIN, AFF_SIGMA_VIRUS, AFF_SENT_CHIP, AFF_SUPERANDROID,
  AFF_EVILBOOST, AFF_EVILSURGE, AFF_EVILOVERLOAD,
  AFF_T1, AFF_T2, AFF_T3, AFF_T4,
  AFF_OMEGA, AFF_VM1, AFF_VM2, AFF_SUPER_OOZARU,
  AFF_SAIBAMAN1, AFF_SAIBAMAN2, AFF_SAIBAMAN3,
  AFF_PUPPET, AFF_SFLAME1, AFF_SFLAME2, AFF_SFLAME3,
  AFF_UFLAME, AFF_ADULT, AFF_DEKU, AFF_GORON, AFF_ZORA, AFF_FDEITY,
  AFF_ACTIVE_FORM, AFF_POWER_FORM, AFF_PROTO_FORM, AFF_OMEGA_FORM,
  AFF_INNERWILL, AFF_MECHAEVO, AFF_ANCIENTEVO,
  AFF_MUTATION1, AFF_MUTATION2, AFF_MUTATION3, AFF_MUTATION4, AFF_MUTATION5,
  AFF_ARAPTURE, AFF_DEMBRACE, AFF_SERALIGHT,
  -1  // Sentinel
};
...
void do_powerdown(CHAR_DATA *ch, char *argument)
{
    ...
    if (wearing_chip(ch))
    {
        send_to_char("You can't while you have a chip installed.\n\r", ch);
        return;
    }
    ...
    if ((xIS_SET(ch->affected_by, AFF_OOZARU) ||
        ...
        ch->exp != ch->pl)
    {
        send_to_char("You can't power down in this form.\n\r", ch);
        return;
    }

    // Restore hair and eyes if in any SSJ-like form
    if (!IS_NPC(ch) &&
    ...
    // Remove all transformation flags
    for (i = 0; transformation_flags[i] != -1; i++)
    {
        if (xIS_SET(ch->affected_by, transformation_flags[i]))
            xREMOVE_BIT(ch->affected_by, transformation_flags[i]);
    }
    ...
    heart_calc(ch, "");
    send_to_char("&BYou power down and return to normal.\n\r", ch);
...
{
    CHAR_DATA *och, *och_next;
    float multipliers[] = {1.1, 1.2, 1.3, 1.4, 1.5, 1.6, 1.7};
    long exp_thresholds[] = {100000, 250000, 500000, 1000000, 2000000, 5000000};
    const int max_powerup = 7;
    ...
    if (IS_AFFECTED(ch, AFF_BIOJR) || wearing_chip(ch) || wearing_sentient_chip(ch)) {
        send_to_char("You can't power up under these conditions.\n\r", ch);
        return;
    }
    ...
    if (!str_cmp(argument, "max")) {
        ...
        if (is_android(ch) || is_superandroid(ch)) {
            ...
            act(auraColor, "Energy nodes across your frame surge with light as you reach your maximum potential.", ch, NULL, NULL, TO_CHAR);
            act(auraColor, "Energy nodes across $n's frame surge with light as $e reaches maximum potential.", ch, NULL, NULL, TO_NOTVICT);
        } else {
        ...
        for (i = 0; i < max_powerup - 1; i++) {
            if (ch->exp <= exp_thresholds[i])
                break;
        }

        ch->powerup = i + 1;
        ch->pl = ch->exp * multipliers[i];
        xREMOVE_BIT(ch->affected_by, AFF_HEART);
        xSET_BIT(ch->affected_by, AFF_POWERUPS);
        heart_calc(ch, "");
        ...
        if (is_splitformed(ch)) {
            for (och = first_char; och; och = och_next) {
                och_next = och->next;
                if (!IS_NPC(och) && is_split(och) && och->master == ch)
    ...
    if (ch->powerup >= max_powerup) {
        send_to_char("&BYou are already powered up to your max.\n\r", ch);
        return;
    }

    if (ch->powerup > 0 && ch->exp <= exp_thresholds[ch->powerup - 1]) {
        send_to_char("&BYou lack the experience to push further.\n\r", ch);
        return;
    }

    // Custom powerup emotes
    const char *emotes_android[] = {
        "A soft hum begins to fill the air as you power up for the first time.",
        "Whirring and grinding noises join the hum as you power up again.",
        "The air around you gains a static charge.",
        "An internal engine roars to life, the ground shaking softly.",
        "A high-pitched whine, cracks forming under your feet.",
        "Engine roaring, electricity crackling everywhere.",
        "The stench of ozone, a deafening boom blasts from you."
    };

    const char *emotes_reploid[] = {
        "Your reactor core pulses rhythmically as you engage power-up mode.",
        "Internal energy matrices align, and your systems intensify.",
        "Sensors flare with light as excess energy is released.",
        "Your feet anchor as magnetized plating stabilizes surges.",
        "Nanocore systems synchronize and push performance limits.",
        "Reactor heat signatures spike as charge flares at your extremities.",
        "Your whole frame resonates with contained digital fury."
    };

    const char *emotes_normal[] = {
        "You begin to glow as you power up for the first time.",
        "You glow brighter as you push your power higher.",
        "You glow intensely, dust swirling around.",
        "The ground trembles beneath you.",
        "Cracks appear in the ground as your power surges.",
        "Rocks fly around as energy explodes outward.",
        "The air itself trembles at your final surge."
    };

    const char *msg;
    if (is_android(ch) || is_superandroid(ch))
        msg = emotes_android[ch->powerup];
    else if (is_reploid(ch))
        msg = emotes_reploid[ch->powerup];
    else
        msg = emotes_normal[ch->powerup];

    act(auraColor, msg, ch, NULL, NULL, TO_CHAR);
    act(auraColor, msg, ch, NULL, NULL, TO_NOTVICT);

    ch->powerup++;
    ch->pl = ch->exp * multipliers[ch->powerup - 1];
    xREMOVE_BIT(ch->affected_by, AFF_HEART);
    xSET_BIT(ch->affected_by, AFF_POWERUPS);
    heart_calc(ch, "");
...
{
    bool is_npc_split = IS_NPC(ch) && is_split(ch);

    if (is_npc_split && (!ch->master || !can_use_skill(ch->master, number_percent(), gsn_ssj)))
        return;

    if (!IS_NPC(ch)) {
        if (IS_SET(ch->pcdata->flags, PCFLAG_KNOWSMYSTIC)) {
            send_to_char("You cannot call upon those powers while you know mystic.\n\r", ch);
            return;
        }
        if (IS_SET(ch->pcdata->flags, PCFLAG_KNOWSLSSJ)) {
            send_to_char("You cannot call upon those powers while possessed by the Legendary Super Saiyan power.\n\r", ch);
            return;
        }
    }

    if (ch->canmajin) {
        send_to_char("You cannot call upon those powers while you know majin.\n\r", ch);
        return;
    }

    if (xIS_SET(ch->affected_by, AFF_SUPER_OOZARU)) {
        send_to_char("You must power out of Super Oozaru first.\n\r", ch);
        return;
    }

    if (wearing_chip(ch) || wearing_sentient_chip(ch)) {
        send_to_char("You can't use this while you have a chip installed.\n\r", ch);
        return;
    }

    if (xIS_SET(ch->affected_by, AFF_KAIOKEN)) {
        send_to_char("You can't transform into a Super Saiyan while using Kaioken.\n\r", ch);
        return;
    }

    // Toggle power down if already in SSJ-based forms
    if (xIS_SET(ch->affected_by, AFF_SSJ) || xIS_SET(ch->affected_by, AFF_SSJ2) ||
        ...
        int ssj_affects[] = {
            AFF_USSJ, AFF_USSJ2, AFF_SSJ, AFF_SSJ2,
            AFF_SSJ3, AFF_SSJ4, AFF_SSJGOD, -1
        };

        for (int i = 0; ssj_affects[i] != -1; i++)
            xREMOVE_BIT(ch->affected_by, ssj_affects[i]);
    ...
    if (!can_use_skill(ch, number_percent(), gsn_ssj)) {
        send_to_char("Your attempt to transform fails as your hair flickers briefly.\n\r", ch);
        learn_from_failure(ch, gsn_ssj);
        return;
    }

    // Success: Transform into SSJ
...
{
    if (IS_NPC(ch) && is_split(ch)) {
        if (!ch->master || !can_use_skill(ch->master, number_percent(), gsn_ssj2))
            return;
    }

    if (!IS_NPC(ch)) {
        if (IS_SET(ch->pcdata->flags, PCFLAG_KNOWSMYSTIC)) {
            send_to_char("You are unable to call upon those powers while you know mystic.\n\r", ch);
            return;
        }
        if (IS_SET(ch->pcdata->flags, PCFLAG_KNOWSLSSJ)) {
            send_to_char("You are unable to call upon those powers while you are possessed with the power of the Legendary Super Saiyan.\n\r", ch);
            return;
        }
    }

    if (ch->canmajin) {
        send_to_char("You are unable to call upon those powers while you know majin.\n\r", ch);
        return;
    }
    ...
    if (xIS_SET(ch->affected_by, AFF_SUPER_OOZARU) ||
        ...
        xIS_SET(ch->affected_by, AFF_GOLDEN_OOZARU)) {
        send_to_char("You must power out of your current form to do this.\n\r", ch);
        return;
    }
    ...
    if (xIS_SET(ch->affected_by, AFF_KAIOKEN)) {
        send_to_char("You can't transform into a Super Saiyan while in Kaioken.\n\r", ch);
        return;
    }
    ...
    int required_mana = !xIS_SET(ch->affected_by, AFF_SSJ) ? skill_table[gsn_ssj2]->min_mana * 1.5 : skill_table[gsn_ssj2]->min_mana;
    if (ch->mana < required_mana) {
        send_to_char("You don't have enough energy.\n\r", ch);
        return;
    }
    ...
    if (!xIS_SET(ch->affected_by, AFF_SSJ)) {
        if (can_use_skill(ch, number_percent(), gsn_ssj2)) {
            act(AT_YELLOW, "Your hair flashes blonde, standing up in a mass of spikes. Your eyes turn blue as a fiery aura bursts into existence around you, quickly becoming charged with sheets of electricity.", ch, NULL, NULL, TO_CHAR);
            act(AT_YELLOW, "$n's hair flashes blonde, standing up in a mass of spikes. $n's eyes turn blue as a fiery aura bursts into existence around $m, quickly becoming charged with sheets of electricity.", ch, NULL, NULL, TO_NOTVICT);
            if (!can_give_light_effect(ch))
                ch->in_room->light++;
            xSET_BIT(ch->affected_by, AFF_SSJ);
        ...
        if (can_use_skill(ch, number_percent(), gsn_ssj2)) {
            act(AT_YELLOW, "Your hair straightens and stands more on end, your aura flaring as electricity arcs around you.", ch, NULL, NULL, TO_CHAR);
            act(AT_YELLOW, "$n's hair straightens and stands more on end, $s aura flaring as electricity arcs around $m.", ch, NULL, NULL, TO_NOTVICT);
            xSET_BIT(ch->affected_by, AFF_SSJ2);
            ch->pl = ch->exp * 18;
            transStatApply(ch, 18, 10, 6, 10);
            learn_from_success(ch, gsn_ssj2);
    ...
    ch->mana -= required_mana;
}

void do_ssj3(CHAR_DATA *ch, char *argument)
{
    if (IS_NPC(ch) && is_split(ch)) {
        if (!ch->master || !can_use_skill(ch->master, number_percent(), gsn_ssj3))
            return;
    }
    ...
    int required_mana = !xIS_SET(ch->affected_by, AFF_SSJ2) ? skill_table[gsn_ssj3]->min_mana * 2 : skill_table[gsn_ssj3]->min_mana;
    if (ch->mana < required_mana) {
        send_to_char("You don't have enough energy.\n\r", ch);
        return;
    }
    ...
    if (!xIS_SET(ch->affected_by, AFF_SSJ2)) {
        if (can_use_skill(ch, number_percent(), gsn_ssj3)) {
            act(AT_YELLOW, "A brilliant golden aura explodes around you as your hair suddenly flashes a similar color. It flows rapidly down your back as your eyebrows disappear.", ch, NULL, NULL, TO_CHAR);
            act(AT_YELLOW, "A brilliant golden aura explodes around $n as $s hair suddenly flashes a similar color. It flows rapidly down $s back as $s eyebrows disappear.", ch, NULL, NULL, TO_NOTVICT);
            if (!can_give_light_effect(ch))
                ch->in_room->light++;
            xSET_BIT(ch->affected_by, AFF_SSJ);
            xSET_BIT(ch->affected_by, AFF_SSJ2);
        ...
        if (can_use_skill(ch, number_percent(), gsn_ssj3)) {
            act(AT_YELLOW, "Your hair lengthens, flowing down your back. Your eyebrows disappear as your aura flashes a bright gold.", ch, NULL, NULL, TO_CHAR);
            act(AT_YELLOW, "$n's hair lengthens, flowing down $s back. $n's eyebrows disappear as $s aura flashes a bright gold.", ch, NULL, NULL, TO_NOTVICT);
            if (!can_give_light_effect(ch))
                ch->in_room->light++;
            xSET_BIT(ch->affected_by, AFF_SSJ3);
            ch->pl = ch->exp * 26;
...
{
    if (IS_NPC(ch) && is_split(ch)) {
        if (!ch->master || !can_use_skill(ch->master, number_percent(), gsn_ssj4))
            return;
    }
    ...
    if (!IS_NPC(ch)) {
        ...
        if (ch->pcdata->tail < 1) {
            send_to_char("You can't become a Super Saiyan 4 without a tail!\n\r", ch);
            return;
        }
        if (ch->pcdata->learned[gsn_ssjgod] > 0) {
            send_to_char("You cannot walk the path of Super Saiyan 4 having already embraced godhood.\n\r", ch);
            return;
        }
    }
    ...
    if (xIS_SET(ch->affected_by, AFF_SUPER_OOZARU) ||
        xIS_SET(ch->affected_by, AFF_OOZARU) ||
        xIS_SET(ch->affected_by, AFF_GOLDEN_OOZARU)) {
        send_to_char("You must power out of your current Oozaru form to do this.\n\r", ch);
        return;
    }
    ...
    int mana_cost = xIS_SET(ch->affected_by, AFF_SSJ3) ? skill_table[gsn_ssj4]->min_mana : skill_table[gsn_ssj4]->min_mana * 3;
    ...
    if (can_use_skill(ch, number_percent(), gsn_ssj4)) {
        ...
        act(AT_BLOOD, "You roar into the sky as your aura detonates in a pillar of writhing crimson fire."
                  "Your muscles surge, your hair blackens and grows wild, and your body becomes wreathed in coarse red fur."
                  "Fiery shadows twist behind you, and your gaze turns feral as your tail lashes with demonic fury.",
            ch, NULL, NULL, TO_CHAR);
        act(AT_BLOOD, "$n roars into the sky as their aura detonates in a pillar of writhing crimson fire."
                  "$n's muscles surge, their hair blackens and grows wild, and their body becomes wreathed in coarse red fur."
                  "Fiery shadows twist behind them, and their gaze turns feral as their tail lashes with demonic fury.",
        ...
        const char *fail_msgs[] = {
            "Your hair flashes black, but quickly returns to blonde.",
            "You almost pop a vein trying to transform into a Super Saiyan 4.",
        ...
        int roll = number_range(0, 3);
        act(AT_BLUE, fail_msgs[roll], ch, NULL, NULL, TO_CHAR);
        act(AT_BLUE, fail_msgs[roll], ch, NULL, NULL, TO_NOTVICT);
...
void do_ssjgod(CHAR_DATA *ch, char *argument)
{
    if (IS_NPC(ch) && is_split(ch)) {
        if (!ch->master || !can_use_skill(ch->master, number_percent(), gsn_ssjgod))
            return;
    }

    if (!IS_NPC(ch)) {
        if (IS_SET(ch->pcdata->flags, PCFLAG_KNOWSMYSTIC)) {
            send_to_char("You cannot ascend while bound to mystic power.\n\r", ch);
            return;
        }
        if (IS_SET(ch->pcdata->flags, PCFLAG_KNOWSLSSJ)) {
            send_to_char("You cannot embrace godhood while possessed by the Legendary Super Saiyan power.\n\r", ch);
            return;
        }
        if (ch->pcdata->learned[gsn_ssj4] > 0) {
            send_to_char("You cannot transcend into godhood having walked the primal path of Super Saiyan 4.\n\r", ch);
            return;
        }
    }

    if (ch->canmajin) {
        send_to_char("You cannot become a god while bound by majin influence.\n\r", ch);
        return;
    }

    if (wearing_chip(ch) || wearing_sentient_chip(ch)) {
        send_to_char("You can't transform while you have a chip installed.\n\r", ch);
        return;
    }

    if (xIS_SET(ch->affected_by, AFF_KAIOKEN)) {
        send_to_char("You can't transform while using Kaioken.\n\r", ch);
        return;
    }

    if (xIS_SET(ch->affected_by, AFF_SSJGOD)) {
        send_to_char("You release your divine power and return to normal.\n\r", ch);
        xREMOVE_BIT(ch->affected_by, AFF_SSJGOD);
        ch->pl = ch->exp;
        transStatRemove(ch);
        if (!IS_NPC(ch)) {
            ch->pcdata->eyes = 0;
    ...
    if (!xIS_SET(ch->affected_by, AFF_SSJ)) level_multiplier++;
    if (!xIS_SET(ch->affected_by, AFF_SSJ2)) level_multiplier++;
    if (!xIS_SET(ch->affected_by, AFF_SSJ3)) level_multiplier++;

    int base_mana_cost = skill_table[gsn_ssjgod]->min_mana * 4;
    int final_mana_cost = base_mana_cost + (base_mana_cost * 15 / 100 * level_multiplier);
    if (final_mana_cost > 99999)
        final_mana_cost = 99999;

    if (ch->mana < final_mana_cost) {
        send_to_char("You lack the spiritual energy to ascend to godhood.\n\r", ch);
        return;
    }

    WAIT_STATE(ch, skill_table[gsn_ssjgod]->beats);

    if (can_use_skill(ch, number_percent(), gsn_ssjgod)) {
        if (!can_give_light_effect(ch))
            ch->in_room->light++;


        act(AT_RED, "You close your eyes... then open them as a brilliant crimson aura engulfs you in divine fire."
                    "Your hair ignites upward in a flowing red flame, and your eyes glow with serene, absolute power.",
            ch, NULL, NULL, TO_CHAR);
        act(AT_RED, "$n's aura bursts into brilliant crimson as divine fire surrounds them."
                    "$n's hair blazes upward like flame and $s eyes glow with calm godly might.",
            ch, NULL, NULL, TO_NOTVICT);

        xSET_BIT(ch->affected_by, AFF_SSJGOD);
        ch->pl = ch->exp * 36;
        transStatApply(ch, 39, 24, 15, 24);

        learn_from_success(ch, gsn_ssjgod);
        if (!IS_NPC(ch)) {
            ch->pcdata->eyes = 6;
            ch->pcdata->haircolor = 1;
        }
    } else {
        act(AT_BLUE, "Your body trembles under the strain of divine power as you fail to ascend. You fall back to Super Saiyan 3.", ch, NULL, NULL, TO_CHAR);
        act(AT_BLUE, "$n's body trembles under the strain of divine power, falling back to Super Saiyan 3.", ch, NULL, NULL, TO_NOTVICT);
        ...
        learn_from_failure(ch, gsn_ssjgod);
    }

    ch->mana -= final_mana_cost;
}
...
void do_fakemoon( CHAR_DATA * ch, char *argument )
{
  ...
  if( xIS_SET( ( ch )->affected_by, AFF_OOZARU) && xIS_SET( ( ch )->affected_by, AFF_FAKEMOON ) )
  {
    send_to_char( "You power down and return to normal.\n\r", ch );
    xREMOVE_BIT( ( ch )->affected_by, AFF_OOZARU );
    xREMOVE_BIT( ( ch )->affected_by, AFF_FAKEMOON );
    ch->pl = ch->exp;
    transStatRemove( ch );
    return;
  }
    if( xIS_SET( ( ch )->affected_by, AFF_OOZARU) && !xIS_SET( ( ch )->affected_by, AFF_FAKEMOON ) )
  {
    send_to_char( "Thats the real moon!\n\r", ch );
     return;
  }
  if( xIS_SET( ( ch )->affected_by, AFF_GOLDEN_OOZARU ) && xIS_SET( ( ch )->affected_by, AFF_FAKEMOON))
  {
    send_to_char( "You power down and return to normal.\n\r", ch );
  ...
  if( can_use_skill( ch, number_percent(  ), gsn_fakemoon ) )
  {
	...
	xSET_BIT( ch->affected_by, AFF_FAKEMOON );
 ...
 //   return;
     ...
     act( z, "$n tries to create a fake moon but it disolves.", ch, NULL, NULL, TO_NOTVICT );
     learn_from_failure( ch, gsn_fakemoon );
	 ch->mana -= skill_table[gsn_fakemoon]->min_mana/2;
     }
...
/* Reworked the SSJ function.*/

/* Shared countdown for SSJ1 */
static void rage1_step(CHAR_DATA *ch, CHAR_DATA *victim, int step)
{
    int dec_color = AT_YELLOW;
    int main_color = AT_RED;
    const char *msg = NULL;

    switch (step) {
        case 6: msg = "You scream out, in pure rage, your hatred for $N."; break;
        case 5:
        case 4: msg = "Your hair flashes blonde, but quickly returns to normal."; break;
        case 3: msg = "Your hair flashes blonde, but slowly fades back to normal."; break;
        case 2: msg = "Your voice bellows out through the area, with a deep angry howl."; break;
        default: return;
    }

    for (int i = 0; i < 2; i++) {
        act(dec_color, "=-", ch, NULL, victim, TO_CHAR);
        act(dec_color, "=-", ch, NULL, victim, TO_VICT);
        act(dec_color, "=-", ch, NULL, victim, TO_NOTVICT);
    }
    act(main_color, msg, ch, NULL, victim, TO_CHAR);
    act(main_color, msg, ch, NULL, victim, TO_VICT);
    act(main_color, msg, ch, NULL, victim, TO_NOTVICT);
    for (int i = 0; i < 2; i++) {
        act(dec_color, "=-", ch, NULL, victim, TO_CHAR);
        act(dec_color, "=-", ch, NULL, victim, TO_VICT);
        act(dec_color, "=-", ch, NULL, victim, TO_NOTVICT);
...
{
    if (!ch->desc || !victim || IS_NPC(ch) || !ch->in_room || !victim->in_room)
        return;
    if (ch->pcdata->learned[gsn_ssj] > 0)
        return;
    if (ch->delay <= 0) {
        if (!is_saiyan(ch) && !is_hb(ch)) return;
        if (ch->rage < 500 || ch->exp < 10000000) return;
        if (number_range(1,5) == 5) {
            int thresh = (ch->rage - 400) / 50;
            if (number_percent() > thresh) {
                const char *flares[] = {
                    "Your hair flashes blonde, but quickly returns to normal.",
                    "You scream out, in pure rage, your hatred for $N.",
                    "Electricity arcs around your body, then fades away."
                };
                const char *m = flares[number_range(0,2)];
                act(AT_BLUE, m, ch, NULL, victim, TO_CHAR);
                act(AT_BLUE, m, ch, NULL, victim, TO_VICT);
                act(AT_BLUE, m, ch, NULL, victim, TO_NOTVICT);
                return;
            }
            ch->delay = 6;
    ...
    if (ch->delay > 1)
        rage1_step(ch, victim, ch->delay);
    else if (ch->delay == 1) {
        act(AT_YELLOW, "Your hair flashes blonde and your eyes turn blue as a fiery aura erupts around you.", ch, NULL, NULL, TO_CHAR);
        act(AT_YELLOW, "$n's hair flashes blonde and $s eyes turn blue as a fiery aura erupts around $m.", ch, NULL, NULL, TO_NOTVICT);
        ch->pcdata->learned[gsn_ssj] = 10;
        if (can_give_light_effect(ch)) ch->in_room->light--;
        xREMOVE_BIT(ch->affected_by, AFF_KAIOKEN);
        xREMOVE_BIT(ch->affected_by, AFF_OOZARU);
        xSET_BIT(ch->affected_by, AFF_SSJ);
        ch->in_room->light++;
...
/* Shared countdown for SSJ2 */
static void rage2_step(CHAR_DATA *ch, CHAR_DATA *victim, int step)
{
    int dec_color = AT_YELLOW;
    int main_color = AT_RED;
    const char *msg = NULL;
    ...
    switch (step) {
        case 6: msg = "You scream out, in pure rage, your hatred for $N."; break;
        case 5: msg = "Electricity begins arcing around your body."; break;
        case 4: msg = "Electricity violently arcs around your body."; break;
        case 3: msg = "Your hair flies back and begins to stand straight up."; break;
        case 2: msg = "Your voice bellows out through the area, with a deep angry howl."; break;
        default: return;
    }
    for (int i=0;i<2;i++) {
...
void rage2(CHAR_DATA *ch, CHAR_DATA *victim)
{
    ...
    /* already learned SSJ2? abort if SSJ1 isn’t fully adept */
    if (ch->pcdata->learned[gsn_ssj2] > 0
     && ch->pcdata->learned[gsn_ssj] < GET_ADEPT(ch, gsn_ssj))
        return;

    /* require SSJ1 at adept cap */
    if (ch->pcdata->learned[gsn_ssj] < GET_ADEPT(ch, gsn_ssj))
        return;

    /* must be currently transformed into SSJ1 */
    if (!xIS_SET(ch->affected_by, AFF_SSJ))
        return;
    ...
    if (ch->delay <= 0)
    {
        ...
        if (number_range(1,5) == 5)
        {
            int thresh = (ch->rage - 800) / 50;
            if (number_percent() > thresh)
            {
                const char *flares[] = {
                    "Electricity begins arcing around your body, but it quickly disappears.",
                    "You scream out, in pure rage, your hatred for $N.",
                    "Electricity violently arcs around your body, then fades away."
                };
                const char *m = flares[number_range(0,2)];
                act(AT_BLUE, m, ch, NULL, victim, TO_CHAR);
                act(AT_BLUE, m, ch, NULL, victim, TO_VICT);
    ...
    if (ch->delay > 1)
        rage2_step(ch, victim, ch->delay);
    else if (ch->delay == 1)
    {
      act(AT_YELLOW,
        "You tremble as grief and rage ignite the slumbering beast.\n"
        "Lightning tears through your aura in jagged storm.\n"
        "Spiked hair glows golden, eyes aflame with fierce resolve.\n"
        "Tears trace determined lines as fury fuels your power.\n"
        "The ground quakes with your unleashed, righteous wrath.\n"
        "You stand reborn by sorrow’s spark, surpassing every limit.",
        ch, NULL, victim, TO_CHAR);
    
    act(AT_YELLOW,
        "$n trembles as grief and rage ignite the slumbering beast within.\n"
        "Lightning tears through $s aura in jagged storm.\n"
        "Spiked hair glows golden, eyes aflame with fierce resolve.\n"
        "Tears trace determined lines on $s face as fury fuels $s power.\n"
        "The ground quakes beneath $n’s unleashed, righteous wrath.\n"
        "$n stands reborn by sorrow’s spark, surpassing every mortal limit.",
        ch, NULL, victim, TO_NOTVICT);
...
/* Shared countdown for SSJ3 */
static void rage3_step(CHAR_DATA *ch, CHAR_DATA *victim, int step)
{
    int dec_color = AT_YELLOW;
    int main_color = AT_RED;
    const char *msg = NULL;
    switch(step) {
        ...
        case 5: msg="Electricity begins arcing around your body."; break;
        case 4: msg="Electricity violently arcs around your body."; break;
        case 3: msg="Your hair begins to flow all the way down your back."; break;
        case 2: msg="Your voice bellows out through the area, with a deep angry howl."; break;
        default: return;
    }
    for(int i=0;i<2;i++){ act(dec_color,"=-",ch,NULL,victim,TO_CHAR); act(dec_color,"=-",ch,NULL,victim,TO_VICT); act(dec_color,"=-",ch,NULL,victim,TO_NOTVICT);}    
    act(main_color,msg,ch,NULL,victim,TO_CHAR);
    act(main_color,msg,ch,NULL,victim,TO_VICT);
    act(main_color,msg,ch,NULL,victim,TO_NOTVICT);
    for(int i=0;i<2;i++){ act(dec_color,"=-",ch,NULL,victim,TO_CHAR); act(dec_color,"=-",ch,NULL,victim,TO_VICT); act(dec_color,"=-",ch,NULL,victim,TO_NOTVICT);}    
...
void rage3(CHAR_DATA *ch, CHAR_DATA *victim)
{
    ...
    /* require SSJ2 at adept cap */
    if (ch->pcdata->learned[gsn_ssj2] < GET_ADEPT(ch, gsn_ssj2))
        return;

    /* must already be in SSJ2 */
    if (!xIS_SET(ch->affected_by, AFF_SSJ2))
        return;
    ...
    if (ch->delay > 1)
    {
        rage3_step(ch, victim, ch->delay);
    }
    else if (ch->delay == 1)
    {
        act(AT_YELLOW,
            "Your brow vanishes as golden mane extends in supernatural length.\n"
            "A thunderous aura swells, crackling with raw, unrestrained power.\n"
            "Massive electricity arcs over your wild, flowing hair like storms.\n"
            "Your roar shakes the earth as muscles strain beyond human limits.\n"
            "Eyes burn with fearless resolve, divine energy radiating outward.\n"
            "You emerge as legend reborn, the Super Saiyan third of myth.",
            ch, NULL, victim, TO_CHAR);

        act(AT_YELLOW,
            "$n's brow vanishes as golden mane extends in supernatural length.\n"
            "A thunderous aura swells around $m, crackling with raw power.\n"
            "Massive electricity arcs over $s wild, flowing hair like storms.\n"
            "$n's roar shakes the earth as muscles strain beyond human limits.\n"
            "Eyes burn in $s gaze with fearless resolve, energy radiating out.\n"
            "$n emerges as legend reborn, the Super Saiyan third of myth.",
            ch, NULL, victim, TO_NOTVICT);
...
/* Shared countdown for SSJ4 */
static void rage4_step(CHAR_DATA *ch, CHAR_DATA *victim, int step)
{
    int dec_color = AT_YELLOW, main_color = AT_RED;
    const char* msg = NULL;
    switch(step) {
        case 6: msg = "You scream out, in pure rage, your hatred for $N."; break;
        case 5: msg = "Electricity violently arcs around your body."; break;
        case 4: msg = "Your form violently shifts, power surging uncontrollably."; break;
        case 3: msg = "Your aura becomes unstable, erupting with divine and primal energies."; break;
        case 2: msg = "Your voice bellows out through the area, with a deep angry howl."; break;
        default: return;
    }
    for (int i = 0; i < 2; i++) {
...
void rage4(CHAR_DATA *ch, CHAR_DATA *victim)
{
    ...
    /* already transformed? */
    if (ch->pcdata->learned[gsn_ssj4]  > 0
     || ch->pcdata->learned[gsn_ssjgod] > 0)
        return;

    /*
     * Require SSJ1, SSJ2 and SSJ3 to be at their adept caps
     * before even starting the rage sequence.
     */
    if (ch->pcdata->learned[gsn_ssj]  < GET_ADEPT(ch, gsn_ssj)
     || ch->pcdata->learned[gsn_ssj2] < GET_ADEPT(ch, gsn_ssj2)
     || ch->pcdata->learned[gsn_ssj3] < GET_ADEPT(ch, gsn_ssj3))
        return;
    ...
    if (ch->delay > 1)
    {
        rage4_step(ch, victim, ch->delay);
    }
    else if (ch->delay == 1)
    {
        /* final transformation tick: SSJ4 vs SSJGOD logic as before */
        if (number_percent() <= 50)
        {
          act(AT_BLOOD,
            "You roar with primordial rage, a savage storm within your blood.\n"
            "Ebony and crimson energy sears from your veins in torrent.\n"
            "Dark fur blooms across your flesh like wildfire in the night.\n"
            "Your claws rake the air as raw power erupts beyond all reason.\n"
            "The ground trembles beneath your thunderous, wrathful roar.\n"
            "You stand a living conflagration of feral, ancient might.",
            ch, NULL, NULL, TO_CHAR);
        
        act(AT_BLOOD,
            "$n roars with primordial rage, a savage storm within $s blood.\n"
            "Ebony and crimson energy sears from $s veins in torrent.\n"
            "Dark fur blooms across $s flesh like wildfire in the night.\n"
            "$s claws rake the air as raw power erupts beyond all reason.\n"
            "The ground trembles beneath $s thunderous, wrathful roar.\n"
            "$n stands a living conflagration of feral, ancient might.",
            ch, NULL, NULL, TO_NOTVICT);
        ...
        {
          act(AT_RED,
            "Crimson hair flows like burning embers around your calm divine form.\n"
            "A scarlet aura flickers with sacred fire in the stillness of your soul.\n"
            "God Ki surges through you, silent yet unyielding as the cosmos.\n"
            "Your frame grows lean and precise, strength honed to perfect focus.\n"
            "Eyes gleam with fiery intent as ancient Saiyan spirit awakens.\n"
            "Awakened by six kindred souls, their power forges your divine spark.",
            ch, NULL, NULL, TO_CHAR);
        
        act(AT_RED,
            "$n's hair flows like burning embers around $s calm divine form.\n"
            "A scarlet aura flickers with sacred fire in the stillness of $s soul.\n"
            "God Ki courses through $n, silent yet unyielding as the cosmos.\n"
            "$s frame grows lean and precise, strength honed to perfect focus.\n"
            "Eyes gleam with fiery intent as ancient Saiyan spirit awakens.\n"
            "Awakened by six kindred souls, their power forges $n's divine spark.",
            ch, NULL, NULL, TO_NOTVICT);

            ch->pcdata->learned[gsn_ssjgod] = 10;
            xREMOVE_BIT(ch->affected_by, AFF_KAIOKEN);
            xREMOVE_BIT(ch->affected_by, AFF_OOZARU);
            xREMOVE_BIT(ch->affected_by, AFF_GOLDEN_OOZARU);
            xSET_BIT(ch->affected_by, AFF_SSJ);
        ...
        /* reset delay and rage, then apply suppression timers */
        ch->delay = 0;
        ch->rage  = 0;
