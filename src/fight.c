...
extern struct rank_tables_list_t rank_lists[];
extern struct rank_table_t rank_table[];
...
//  clan = victim->pcdata->clan;
  ...
  {
    if(ch->race == victim->race && IS_RANKED(victim) && ch->rank <= victim->rank)
      clanbypass = TRUE;
  ...
  if( xIS_SET( victim->act, PLR_PK1 ) || ( xIS_SET( victim->act, PLR_WAR1 ) && !IS_HC( victim ) ) )
  {
  ...
  if( IS_RANKED( ch ) && IS_RANKED( victim ) )
  {
  ...
  if( !xIS_SET( victim->act, PLR_PK1 ) && !xIS_SET( victim->act, PLR_PK2 )
      && !xIS_SET( victim->act, PLR_WAR1 ) && !xIS_SET( victim->act, PLR_WAR2 ) && !IS_HC( victim ) )
  {
...
*/
    ...
    {
      int div = 0;
      if( ch->pcdata->learned[gsn_dodge] > 0 && !IS_SET( ch->pcdata->combatFlags, CMB_NO_DODGE ) )
        div += 5;
      if( ch->pcdata->learned[gsn_block] > 0 && !IS_SET( ch->pcdata->combatFlags, CMB_NO_BLOCK ) )
        div += 5;
      if( ch->pcdata->learned[gsn_dcd] > 0 && !IS_SET( ch->pcdata->combatFlags, CMB_NO_DCD ) )
        div += 40;
     ...
     * log_string( buf );
     * //Used to prevent people from killing in a safe room... not needed anymore. - 05.03.06
    ...
    if( victim->tmystic > 0 )
    {
      sprintf( buf, "violence_update: %s fighting %s in a SAFE room while teaching mystic.", ch->name, victim->name );
      log_string( buf );
      stop_fighting( ch, TRUE );
    }
...
*/
  ...
  chance = IS_NPC( ch ) ? ch->level : ( int )( ( LEARNED( ch, gsn_tail_attack ) + dual_bonus ) / 1.5 );
  if( number_percent(  ) < chance )
  {
  ...
  chance = IS_NPC( ch ) ? ch->level : ( int )( ( LEARNED( ch, gsn_second_attack ) + dual_bonus ) / 1.5 );
  if( number_percent(  ) < chance )
  {
  ...
  chance = IS_NPC( ch ) ? ch->level : ( int )( ( LEARNED( ch, gsn_third_attack ) + ( dual_bonus * 1.5 ) ) / 2 );
  if( number_percent(  ) < chance )
  {
  ...
  chance = IS_NPC( ch ) ? ch->level : ( int )( ( LEARNED( ch, gsn_fourth_attack ) + ( dual_bonus * 2 ) ) / 3 );
  if( number_percent(  ) < chance )
  {
  ...
  chance = IS_NPC( ch ) ? ch->level : ( int )( ( LEARNED( ch, gsn_fifth_attack ) + ( dual_bonus * 3 ) ) / 4 );
  if( number_percent(  ) < chance )
  {
...
*/
  ...
  /*
   * For NPC's, they can be set to automatically take a % off dammage
   */
  if( IS_NPC( victim ) )
  {
    if( ( GET_AC( victim ) > 0 ) )
 ...
 */
ch_ret projectile_hit( CHAR_DATA * ch, CHAR_DATA * victim, OBJ_DATA * wield, OBJ_DATA * projectile, sh_int dist )
{
  int victim_ac;
  int thac0;
  int thac0_00;
...
//  char kbuf[MAX_STRING_LENGTH];
  char filename[256];
  int i, rl, ti1, ti2, ti3, ti4, victim_r, killer_r;
  bool npcvict;
  bool kaiopres = FALSE;
  bool demonpres = FALSE;
  bool saiyanpres = FALSE;
  bool loot;
/*    int  xp_gain = 0;
    int  xp_gain_post = 0;*/
...
//  clan = victim->pcdata->clan;
   ...
   * a way out. --Shaddai
   * Revised for rank pking in safe rooms -Karn 05.17.06
   */
//  if( xIS_SET( victim->in_room->room_flags, ROOM_SAFE )  )
//    dam = 0;
...
*/
  ...
  if( victim != ch )
  {
    ...
    if( !IS_NPC( victim ) && ch->melee )
    {
      ...
      if( victim->pcdata->learned[gsn_dodge] > 0 && !IS_SET( victim->pcdata->combatFlags, CMB_NO_DODGE ) )
      {
      ...
      if( victim->pcdata->learned[gsn_block] > 0 && !victim->dodge && !IS_SET( victim->pcdata->combatFlags, CMB_NO_BLOCK ) )
      {
  ...
  /* Fix to adjust for when negative ki's jump into here -- Chili */
  victim->mana = UMAX(victim->mana, 0);

  heart_calc( victim, "" );


  if( !IS_NPC( victim ) && !IS_IMMORTAL( victim ) && !is_hb_h( victim ) && !is_saiyan_h( victim ) )
  {
        ...
        || ( victim->pcdata->learned[gsn_ssj3] > 0 && victim->pcdata->learned[gsn_ssj4] <= 0 && victim->exp >= 2000000000 ) )
    {
      if( IS_NPC( ch ) || ( !IS_NPC( ch ) && !xIS_SET( ch->act, PLR_SPAR ) ) )
      {
        ...
        if( victim->exp >= 8000000 && victim->pcdata->learned[gsn_ssj] <= 0 )
          rage( victim, ch );
        else if( victim->exp >= 50000000 && victim->pcdata->learned[gsn_ssj] > 0 && victim->pcdata->learned[gsn_ssj2] <= 0 )
          rage2( victim, ch );
        else if( victim->exp >= 500000000
                 && victim->pcdata->learned[gsn_ssj2] > 0 && victim->pcdata->learned[gsn_ssj3] <= 0 )
          rage3( victim, ch );
        else if( victim->exp >= 2000000000
                 && victim->pcdata->learned[gsn_ssj3] > 0 && victim->pcdata->learned[gsn_ssj4] <= 0 )
          rage4( victim, ch );
      }
    }
  }
      ...
      !IS_IMMORTAL( ch ) && !IS_IMMORTAL( victim ) && !is_split( victim ) )
  {
     ...
     *
     * Me too!!!! OMGZ0R! -Karn 01.14.05
     */

    /*
     * prevents a lot of overflow pl gains -Goku 09.28.04
     * 
     * na, fuck that... we like overkill -Karn 01.12.05
     */
    if( dam > 100 )
      dam = 100;
     ...
     * *
     * * lowered them another 0.005 -Karn 01.14.05
     */
    if( ch->race != 6 )
    {
...
/* A little help to get newbies started */
    ...
    xp_gain += (xp_gain * weightedClothingPlMod(ch));
...
//      && number_range(1, 100) < 35)
    ...
    xp_gain *= 3; // Gain-modding section.
    ...
    else if( is_fierian( ch ) )
    {
      ...
      {
        if( !is_leet( ch ) )
          sprintf( buf1, "Your hl increases by %s points.", num_punct_ld( xp_gain_post ) );  
        else
          sprintf( buf1, "Omgawd u git %s atche el. hawt!", num_punct_ld( xp_gain_post ) );
        act( AT_HIT, buf1, ch, NULL, victim, TO_CHAR );
      }
      else
      {
        sprintf( buf1, "Your hl increases by %s point.", num_punct_ld( xp_gain_post ) );
        act( AT_HIT, buf1, ch, NULL, victim, TO_CHAR );
      }
    }
    else if( is_hylian( ch ) )
    {
      ...
      {
        if( !is_leet( ch ) )
          sprintf( buf1, "Your exp increases by %s points.", num_punct_ld( xp_gain_post ) );  
        else
          sprintf( buf1, "Omgawd u git %s eeee ecks peee. hawt!", num_punct_ld( xp_gain_post ) );
        act( AT_HIT, buf1, ch, NULL, victim, TO_CHAR );
      }
      else
      {
        sprintf( buf1, "Your exp increases by %s point.", num_punct_ld( xp_gain_post ) );
        act( AT_HIT, buf1, ch, NULL, victim, TO_CHAR );
      }
    }
    else if( is_reploid( ch ) )
    {
      ...
      {
        if( !is_leet( ch ) )
          sprintf( buf1, "Your power increases by %s points.", num_punct_ld( xp_gain_post ) );  
        else
          sprintf( buf1, "Omgawd u git %s POWR. hawt!", num_punct_ld( xp_gain_post ) );
        act( AT_HIT, buf1, ch, NULL, victim, TO_CHAR );
      }
      else
      {
        sprintf( buf1, "Your power increases by %s point.", num_punct_ld( xp_gain_post ) );
        act( AT_HIT, buf1, ch, NULL, victim, TO_CHAR );
      }
    }
    else
  ...
  if( victim->position == POS_DEAD )
  {
    ...
    /* New Rank code by Chili (RLB) */
    /* They get to take their rank when they kill 'em */
    if(IS_RANKED(victim) && !IS_NPC(ch) && (rl = get_rank_list_num(ch)) > -1 
	&&  (rank_lists[rl].race(victim) && ch->rank < victim->rank)) {
      int high_rank;
      /* Find out which race they are */
      for(i = 0; rank_lists[i].name != NULL && !rank_lists[i].race(ch); i++)
	;
      if(rank_lists[i].name != NULL) {
	sprintf(kbuf, "%s has stolen %s's title of %s and is now the %s.", ch->name, victim->name, mud_capstr(rank_lists[i].name), 
	    get_ch_rank_at(ch, victim->rank));
	to_channel(kbuf, CHANNEL_MONITOR, "Monitor", LEVEL_IMMORTAL);
	do_info(ch, kbuf);
	for(victim_r = 0;  victim_r < rank_lists[i].high ; victim_r++)
	  if(!str_cmp(victim->name, rank_lists[i].rank[victim_r]))
	    break;
	/* Remove effects of rank if any */
	ti2 = rank_lists[i].high - victim_r;
	for(ti1 = 0 ; rank_table[ti1].rank_name_m != NULL ; ti1++) {
	  if(rank_table[ti1].race(victim))
	    ti2--;
	  else 
	    continue;
	  if(ti2 == 0)
	...
	if(rank_table[ti1].func_loss != NULL)
	  rank_table[ti1].func_loss(victim);

	if(victim_r < rank_lists[i].high) {
	  /* Find out which rank each guy is */
	  if(IS_RANKED(ch)) { /* The guy that killed them is at least ranked, they get to at least fall back to their title */
	    for(killer_r = 0;  killer_r < rank_lists[i].high ; killer_r++)
	      if(!str_cmp(ch->name, rank_lists[i].rank[killer_r]))
		break;
	    if(killer_r >= rank_lists[i].high)
	      goto FIGHT_NON_KILLER_RANK;
	    /* Remove effects of rank if any */
	    ti4 = rank_lists[i].high - killer_r;
	    for(ti3 = 0 ; rank_table[ti3].rank_name_m != NULL ; ti3++) {
	      if(rank_table[ti3].race(ch))
		ti4--;
	      else 
		continue;
	      if(ti4 == 0)
	    ...
	    if(rank_table[ti3].func_loss != NULL)
	      rank_table[ti3].func_loss(ch);

	    /* Now swap ranks */
	    if(rank_table[ti1].func_gain != NULL)
	      rank_table[ti1].func_gain(ch);
	    if(rank_table[ti3].func_gain != NULL)
	      rank_table[ti3].func_gain(victim);
	    /* Apply killers rank to victim */
	    high_rank = victim->rank;
	    victim->rank = ch->rank;
	    ch->rank = high_rank;
	    /* Announce they now have that rank */
	    sprintf(kbuf, "%s has dropped to the %s title of %s", victim->name, mud_capstr(rank_lists[i].name), get_ch_rank(victim));
	    do_info(ch, kbuf);
	    STRFREE(rank_lists[i].rank[victim_r]) ;
	    STRFREE(rank_lists[i].rank[killer_r]);
	    rank_lists[i].rank[victim_r] = STRALLOC(ch->name); 
	    rank_lists[i].rank[killer_r] = STRALLOC(victim->name); 
	    /* Now change their spots in the table */
	    save_char_obj(ch);
	    save_char_obj(victim);
	    save_sysdata(sysdata);
	  } else {
FIGHT_NON_KILLER_RANK:
	    if((victim_r == (ti2-1)) && rank_table[ti2].func_gain != NULL)
	      rank_table[ti1].func_gain(ch);
	    ...
	    ch->rank = victim->rank;
	    victim->rank = 0;
	    sprintf(kbuf, "%s is a weak %s", victim->name, mud_capstr(rank_lists[i].name));
	    do_info(ch, kbuf);
	    save_char_obj(ch);
	    save_char_obj(victim);
	    save_sysdata(sysdata);
	  }
	} else bug("ERROR DETECTED AT %s:%d", __FILE__,  __LINE__);
      }
    }
   
   if( !IS_NPC( ch ) && ( is_bio( ch ) || is_android( ch ) || is_superandroid( ch ) )
        && ch->pcdata->learned[gsn_self_destruct] > 0 )
    {
      if( ch->exp > ch->pl )
      {
        if( IS_NPC( victim ) )
    ...
    if( !npcvict )
    {
      ...
      if( !IS_NPC( ch ) && !IS_NPC( victim ) )
      {
        ...
        if( !str_cmp( victim->name, ch->pcdata->hunting )
            && xIS_SET( victim->act, PLR_BOUNTY ) && victim->pcdata->bounty > 0 )
        {
      ...
      sprintf( buf, "%s has been &zkilled&R by %s", victim->name, ( IS_NPC( ch ) ? ch->short_descr : ch->name ) );
      do_info( ch, buf );
//      if( ch->pcdata->clan == victim->pcdata->clan  )
//      {
//        sprintf(buf2, "&C%s&R has taken control of the clan controlled by &C%s&R!", ch->name, victim->name );
//        do_info(ch, buf2);
//        STRFREE( clan->owner );
//        clan->owner = STRALLOC( ch->name );
//        save_clan( clan );
//      }
          ...
          && ch->pcdata->clan->clan_type != CLAN_ORDER && ch->pcdata->clan->clan_type != CLAN_GUILD && victim != ch )
      {
        ...
        sprintf( log_buf, "&P(%2d) %-12s &wvs &P(%2d) %s &P%s ... &w%s",
                 ...
                 "&P(&WRonin&P)", victim->name, ch->in_room->area->name );
 ...
 *
 * Eh... Warren told me to edit this for pl loss on death...
 * so here goes... this makes #3 -Karn 01.29.05
 * 
 * Stablizing -Karn 04.09.05
 *
 * Reverted back to the original because Silber made a mistake
 * when he allowed super high gains... Totally killed the mud.
 * Don't EVER do it...   - Karn 09/27/05
 *
 * Dummy down version caused a change of plans. reverting back
 * to the high gain stuff. - Karn 03.30.06
 */

      if( !IS_IMMORTAL( ch ) && !IS_IMMORTAL( victim ) )
      {
        if( can_use_skill( victim, number_percent(  ), gsn_preservation ) && !immortal )
        {
        ...
        if( can_use_skill( victim, number_percent(  ), gsn_regeneration ) && !immortal )
        {
...
/*		  if (ch->pcdata->clan && victim->pcdata->clan
		    && allianceStatus(ch->pcdata->clan, victim->pcdata->clan) == ALLIANCE_ATWAR)
		  {
		    if (preservation || biopres )
		      los = (long double)victim->exp * 0.5;
		    else
		      los = (long double)victim->exp * 0.5;
		  }
		  else
...
*/
          ...
          if( ( preservation && !immortal ) || ( biopres && !immortal ) )
          {
            if( warpres || immortal )
              los = ( long double )victim->exp * 0.005;
            else if( IS_RANKED(ch) || kaiopres || demonpres || saiyanpres )
              los = ( long double )victim->exp * 0.0055;
            else
              los = ( long double )victim->exp * 0.006;
          }
          else
          {
            if( warpres || immortal )
              ...
              los = ( long double )victim->exp * 0.0065;
            else
              los = ( long double )victim->exp * 0.007;
          }
        }
        else
        {
          ...
          else
          {
            if( immortal )
              ...
              los = ( long double )victim->exp * 0.008;
          }
        }
    ...
    if( !IS_NPC( victim ) && ( preservation || biopres || immortal ) )
    {
      ...
      for( ;; )
      {
        ...
        if( pRoomIndex )
        {
          if( !xIS_SET( pRoomIndex->room_flags, ROOM_PRIVATE )
              ...
              && !xIS_SET( pRoomIndex->room_flags, ROOM_PROTOTYPE ) && has_exits( pRoomIndex ) )
          {
            break;
          }
        }
        /*
         * Bugfix by Sigma. Be gone, infinite loop crashes! 
         */
        if( count >= 100000 )
        {
      ...
      ROOM_INDEX_DATA *temp = victim->in_room;

      if( preservation )
      {
        act( AT_RED, "With $n's final ounce of energy, $n coughs up an egg and launches it in to the air!", victim, NULL,
             NULL, TO_ROOM );
        send_to_char( "&RWith your final ounce of energy, you cough up an egg and launch it in to the air!", victim );
        char_from_room( victim );
        char_to_room( victim, pRoomIndex );
        victim->position = POS_STANDING;
        act( AT_MAGIC, "An egg falls from the sky creating a small crater as it hits the ground.", victim, NULL, NULL,
             TO_ROOM );
        act( AT_MAGIC, "The egg begins to crack open as $n pops out!", victim, NULL, NULL, TO_ROOM );
        act( AT_MAGIC, "Your egg lands with a THUD!  You break free of your egg and emerge reborn!", victim, NULL, NULL,
             TO_CHAR );
      }
      else if( biopres )
      {
...
bool is_safe( CHAR_DATA * ch, CHAR_DATA * victim, bool show_messg )
{
  ...
  if( IS_RANKED(ch) && is_fighting(ch) )
    return FALSE;
 ...
 */
  ...
  {
    /* Define NO_PKTIMER, and the 30 minute wait don't exist. :) */
#ifndef NO_PKTIMER
      ...
      && IS_SET( ch->pcdata->flags, PCFLAG_DEADLY ) && IS_SET( victim->pcdata->flags, PCFLAG_DEADLY ) )
  {
             ...
             && victim->pcdata->clan->clan_type != CLAN_NOKILL && ch->pcdata->clan != victim->pcdata->clan ) )
    {
      ...
      if( victim->pcdata->pk_timer > 0 )
        victim->pcdata->pk_timer = 0;
      if( !IS_HC( victim ) )
        add_timer( victim, TIMER_PKILLED, 230, NULL, 0 );
      else
        add_timer( victim, TIMER_PKILLED, 230, NULL, 0 );
  ...
  if( is_splitformed( victim ) )
  {
    ...
    for( och = first_char; och; och = och_next )
    {
      ...
      if( ( xIS_SET( och->affected_by, AFF_SPLIT_FORM )
            ...
            || xIS_SET( och->affected_by, AFF_BIOJR ) ) && och->master == victim )
      {
...
void do_stopspar( CHAR_DATA * ch, char *argument )
{
  ...
  cspar = ch->exp - ch->spar_start;
  vspar = victim->exp - victim->spar_start;
  ch_printf( ch, "&cTotal %s gained this spar: &C%s\n\r", ( ( is_android( ch ) ) ? "tl" : "pl" ), num_punct_ld( cspar ) );
  ch_printf( victim, "&cTotal %s gained this spar: &C%s\n\r",
             ( ( is_android( ch ) ) ? "tl" : "pl" ), num_punct_ld( vspar ) );
...
*/
  ...
  if( !IS_NPC( ch ) && !IS_NPC( victim ) )
    if( !pkill_ok( ch, victim ) && !xIS_SET( victim->act, PLR_BOUNTY ) )
      return;
...
//      if( ch->pcdata->clan == victim->pcdata->clan )
//      {
//	 sprintf(buf2, "&C%s&R has challenged &C%s&R for control of their clan!", ch->name, victim->name );
//	 do_info(ch, buf2);
//      }
      if( IS_PKILL( victim ) )
      {
...
bool check_illegal_pk( CHAR_DATA * ch, CHAR_DATA * victim )
{
  ...
  if( !IS_NPC( victim ) && !IS_NPC( ch ) )
  {
    if( ( !IS_SET( victim->pcdata->flags, PCFLAG_DEADLY )
          || !IS_SET( ch->pcdata->flags, PCFLAG_DEADLY ) ) && !in_arena( ch ) && ch != victim )
    {
      if( IS_NPC( ch ) )
...
*/
  ...
  if( !IS_NPC( wf ) && IS_DEMON( ch ) && IS_DEMON( wf ) && ch->rank > 0 && ch->rank > wf->rank )
  {
    ch_printf( ch,
               "You can't flee from a rank duel with a person of lower rank! You have to defend your title to the death!\n\r" );
    return;
  }

  if( !IS_NPC( wf ) && IS_KAIO( ch ) && IS_KAIO( wf ) && ch->rank > 0 && ch->rank > wf->rank )
  {
    ch_printf( ch,
               "You can't flee from a rank duel with a person of lower rank! You have to defend your title to the death!\n\r" );
    return;
  ...
  if( !IS_NPC( wf ) && is_saiyan( ch ) && is_saiyan( wf ) && ch->rank > 0 && ch->rank > wf->rank )
  {
    ch_printf( ch,
               "You can't flee from a rank duel with a person of lower rank! You have to defend your title to the death!\n\r" );
    return;
...
/*	if(!IS_NPC(wf) && is_saiyan(ch) && is_saiyan(wf) && ch->saiyanrank > 0 &&
	ch->saiyanrank > wf->saiyanrank )
    {
	ch_printf(ch,"You can't flee from a rank duel with a person of lower rank! You have to defend your title to the death!\n\r");
	return;
    }*/
...
/* Warren  -- This is for future referance about fleeing pl loss -- Karn*/
...
void do_slay( CHAR_DATA * ch, char *argument )
{
  ...
  if( arg[0] == '\0' )
  {
    ...
    send_to_char( "Types: Skin, Slit, Immolate, Demon, Shatter, 9mm, Deheart, Pounce, Cookie, Fslay, Lightning; *NEW ADDITIONS* testify, fakebalzhur, testlightning, shoes, dog(new hidden one Link found).\n\r", ch );
    return;
  }
  ...
  else if( !str_cmp( arg2, "skin" ) )
  {
    ...
    act( AT_BLOOD, "$n rips the flesh off of $N, releasing his soul into the fiery depths of hell.", ch, NULL, victim,
         TO_NOTVICT );
  }
  ...
  else if( !str_cmp( arg2, "deheart" ) )
  {
    ...
    act( AT_BLOOD, "You feel a sharp pain as $n rips into your chest and pulls our your beating heart in $M hand.", ch, NULL,
         victim, TO_VICT );
    act( AT_BLOOD, "Specks of blood hit your face as $n rips through $N's chest pulling out $M's beating heart.", ch, NULL,
         victim, TO_NOTVICT );
  }
  ...
  else if( !str_cmp( arg2, "lightning" ) )
  {
    ...
    act( AT_RED, "A holy lightning bolt from the skies above reduces you to blasphemous cinders.", ch, NULL, victim,
         TO_VICT );
    act( AT_RED, "$N is reduced to blasphemous cinders by a lightning bolt.", ch, NULL, victim, TO_NOTVICT );
  }
  else if( !str_cmp( arg2, "fakebalzhur" ) )
  {
    act( AT_IMMORT, "You points your finger at $N and nods to Balzhur. Balzhur grins and screams, You are MINE $N!!! and lobs $N's head off!", ch, NULL, victim, TO_CHAR );
    act( AT_IMMORT, "$n points his finger at you and nods to Balzhur. Balzhur grins and screams, You are MINE $N!!! and lobs your head off!", ch, NULL, victim,
         TO_VICT );
    act( AT_IMMORT, "$n points his finger at $N and nods to Balzhur. Balzhur grins and screams, You are MINE $N!!! and lobs $N's head off!", ch, NULL, victim, TO_NOTVICT );
  }
  else if( !str_cmp( arg2, "testlightning" ) )
  {
    act( AT_RED, "You do some FUNKY shit and KEEL $N.", ch, NULL, victim, TO_CHAR );
    act( AT_RED, "J00 JUST GOT KNOCKED THE FUCK OUT!!", ch, NULL, victim,
         TO_VICT );
    act( AT_RED, "$n does some FUNKY shit and KEELS $N", ch, NULL, victim, TO_NOTVICT );
  }

  if( !str_cmp( arg2, "testify" ) )
  {
    act( AT_IMMORT, "&r                               &W***", ch, NULL, victim, TO_CHAR );
    act( AT_IMMORT, "&r                               &W***     &z***", ch, NULL, victim, TO_CHAR );
    act( AT_IMMORT, "&r *********** &R******* &z****  &w***********     &R***** &r** **&D", ch, NULL, victim, TO_CHAR );
    act( AT_IMMORT, "&r **  ***  ** &R**      &z**    &w*********** &z*** &R*     &r** **&D", ch, NULL, victim, TO_CHAR );
    act( AT_IMMORT, "&r     ***     &R****    &z  **  &W    ***     &z*** &R***     &r*&D", ch, NULL, victim, TO_CHAR );
    act( AT_IMMORT, "&r     ***     &R**      &z****  &W    ***     &z*** &R***     &r*&D", ch, NULL, victim, TO_CHAR );
    act( AT_IMMORT, "&r     ***     &R*******         &W  ***     &z*** &R*       &r*&D", ch, NULL, victim, TO_CHAR );
    act( AT_IMMORT, "&r                               &W***", ch, NULL, victim, TO_CHAR );
    act( AT_IMMORT, "&r                               &W***", ch, NULL, victim, TO_CHAR );
    act( AT_IMMORT, "&r                               &W***", ch, NULL, victim, TO_CHAR );
    act( AT_IMMORT, "&r                               &W***", ch, NULL, victim, TO_VICT );
    act( AT_IMMORT, "&r                               &W***     &z***", ch, NULL, victim, TO_VICT );
    act( AT_IMMORT, "&r *********** &R******* &z****  &w***********     &R***** &r** **&D", ch, NULL, victim, TO_VICT );
    act( AT_IMMORT, "&r **  ***  ** &R**      &z**    &w*********** &z*** &R*     &r** **&D", ch, NULL, victim, TO_VICT );
    act( AT_IMMORT, "&r     ***     &R****    &z  **  &W    ***     &z*** &R***     &r*&D", ch, NULL, victim, TO_VICT );
    act( AT_IMMORT, "&r     ***     &R**      &z****  &W    ***     &z*** &R***     &r*&D", ch, NULL, victim, TO_VICT );
    act( AT_IMMORT, "&r     ***     &R*******         &W  ***     &z*** &R*       &r*&D", ch, NULL, victim, TO_VICT );
    act( AT_IMMORT, "&r                               &W***", ch, NULL, victim, TO_VICT );
    act( AT_IMMORT, "&r                               &W***", ch, NULL, victim, TO_VICT );
    act( AT_IMMORT, "&r                               &W***", ch, NULL, victim, TO_VICT );
    act( AT_IMMORT, "&r                               &W***", ch, NULL, victim, TO_NOTVICT );
    act( AT_IMMORT, "&r                               &W***     &z***", ch, NULL, victim, TO_NOTVICT );
    act( AT_IMMORT, "&r *********** &R******* &z****  &w***********     &R***** &r** **&D", ch, NULL, victim, TO_NOTVICT );
    act( AT_IMMORT, "&r **  ***  ** &R**      &z**    &w*********** &z*** &R*     &r** **&D", ch, NULL, victim, TO_NOTVICT );
    act( AT_IMMORT, "&r     ***     &R****    &z  **  &W    ***     &z*** &R***     &r*&D", ch, NULL, victim, TO_NOTVICT );
    act( AT_IMMORT, "&r     ***     &R**      &z****  &W    ***     &z*** &R***     &r*&D", ch, NULL, victim, TO_NOTVICT );
    act( AT_IMMORT, "&r     ***     &R*******         &W  ***     &z*** &R*       &r*&D", ch, NULL, victim, TO_NOTVICT );
    act( AT_IMMORT, "&r                               &W***", ch, NULL, victim, TO_NOTVICT );
    act( AT_IMMORT, "&r                               &W***", ch, NULL, victim, TO_NOTVICT );
    act( AT_IMMORT, "&r                               &W***", ch, NULL, victim, TO_NOTVICT );
  }

  if( !str_cmp( arg2, "shoes" ) )
  {
    act( AT_RED, "You exclaim, OH LOOK! LOOK AT THEIR FUCKIN SHOES; $N'S FUCKIN SHOES FLEW OFF!!", ch, NULL, victim, TO_CHAR );
    act( AT_RED, "OMGZOX YOU JUST LOST YOUR FUCKIN SHOES.", ch, NULL, victim,
         TO_VICT );
    act( AT_RED, "$n exclaims, OH LOOK! LOOK AT THEIR FUCKIN SHOES; $N'S FUCKIN SHOES FLEW OFF!!", ch, NULL, victim, TO_NOTVICT );
  }
...
{
  {
    ...
    {
      act( AT_BLOOD, "You rip through $N's chest and pull out $M beating heart in your hand.", ch, NULL, victim, TO_CHAR );
      act( AT_BLOOD, "You feel a sharp pain as $n rips into your chest and pulls our your beating heart in $M hand.", ch,
           NULL, victim, TO_VICT );
      act( AT_BLOOD, "Specks of blood hit your face as $n rips through $N's chest pulling out $M's beating heart.", ch, NULL,
           victim, TO_NOTVICT );
    }
...
void kaioken_drain(CHAR_DATA *ch)
{
    ...
    {
        act(AT_RED, "The powers of your transformation fade as you lose consciousness.", ch, NULL, NULL, TO_CHAR);
        act(AT_RED, "$n's transformation fades as $e loses consciousness.", ch, NULL, NULL, TO_NOTVICT);
    ...
    {
        int kaioken = ch->pl / ch->exp;
        int kdrain = number_range(6, 8) * kaioken;
        if (!str_cmp(get_race(ch), "kaio"))
            kdrain /= 3;
        drain += kdrain;
    }

    // SSJ drains and skill checks
    #define DRAIN_SKILL(form, gsn, max_cost) \
        if (xIS_SET(ch->affected_by, form)) { \
            int skill = ch->pcdata->learned[gsn]; \
            drain += (int)((max_cost * (100 - skill)) / 100.0); \
            if (skill < 100 && number_percent() < (10 + (100 - skill) / 5)) \
                ch->pcdata->learned[gsn]++; \
        }

    DRAIN_SKILL(AFF_SSJ, gsn_ssj, 100);
    DRAIN_SKILL(AFF_SSJ2, gsn_ssj2, 400);
    DRAIN_SKILL(AFF_SSJ3, gsn_ssj3, 700);
    DRAIN_SKILL(AFF_SSJ4, gsn_ssj4, 1000);
    DRAIN_SKILL(AFF_USSJ, gsn_ssj2, 300);
    DRAIN_SKILL(AFF_USSJ2, gsn_ssj3, 600);
    DRAIN_SKILL(AFF_SSJGOD, gsn_ssjgod, 1000);
    ...
    if (drain != 0 && ch->mana == 0)
    {
        if (xIS_SET(ch->affected_by, AFF_KAIOKEN))
        {
            ...
            else
            {
                ...
                {
                    act(AT_RED, "Your body explodes under the strain of your transformation.", ch, NULL, NULL, TO_CHAR);
                    act(AT_RED, "$n explodes under the strain of transformation.", ch, NULL, NULL, TO_NOTVICT);
                    sprintf(buf, "%s explodes under the strain of transformation.", ch->name);
                    do_info(ch, buf);
                    if (!IS_NPC(ch))
                        los = ch->exp * 0.0085;
                    gain_exp(ch, 0 - los);
        ...
        {
            act(AT_RED, "Your energy fades, and you revert from your transformation.", ch, NULL, NULL, TO_CHAR);
            act(AT_RED, "$n's energy fades, and they revert from their transformation.", ch, NULL, NULL, TO_NOTVICT);
