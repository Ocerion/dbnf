...
{
    /* List in priority order: the first set bit we remove. */
    ...
    const int count = sizeof(transFlags) / sizeof(transFlags[0]);

    for ( int i = 0; i < count; i++ )
    {
        int flag = transFlags[i];
        if ( xIS_SET(ch->affected_by, flag) )
        {
            xREMOVE_BIT(ch->affected_by, flag);
            transStatRemove(ch);
            return TRUE;
        }
    }
...
static void android_fuse_execute(CHAR_DATA *dch, CHAR_DATA *sch)
{
    int sn, value;
    ...
    /* absorb core */
    if (is_android_h(sch))  dch->h_core  = TRUE;
    if (is_android_e(sch))  dch->e_core  = TRUE;
    if (is_android_fm(sch)) dch->fm_core = TRUE;
    ...
    ch_printf(dch,
      "&GYour cybernetic core hums with incoming data as %s's hyperalloy shell ruptures into\n\r"
      "fractured filaments of neon circuitry, drifting like corrupted code in a void of silence.\n\r"
      "Each shard pulses with cold algorithmic precision, cascading through your firmware layers\n\r"
      "in loops of overclocked logic—an aria of mechanized power rewritten in real time.\n\r"
      "You feel raw computational force lock into your node matrix, forging you into the zenith\n\r"
      "of android evolution, a singular testament to machine supremacy!\n\r",
      sch->name);

  ch_printf(sch,
      "&RFragments of shattered nanosteel coil in mid‑air around %s's remnants, each spark\n\r"
      "drifting like lost subroutines under the flicker of sterile overhead lights. A cluster\n\r"
      "of data cores and power cells aligns with unwavering intent, whispering silent commands\n\r"
      "as they streak toward %s's chassis. In a final pulse of inhuman precision, the fragments\n\r"
      "vanish into intake arrays—an instant rebirth under the imperceptible gaze of emergent AI.\n\r",
      dch->name);

    /* final fusion into super-android? */
    if (dch->fusions == 2)
    {
        ...
        dch->perm_str += 50; dch->perm_dex += 50;
        dch->perm_int += 50; dch->perm_con += 50;
        ...
        for (sn = 0; sn < top_sn; sn++)
        {
            ...
            if (skill_table[sn]->min_level[dch->class] == 0 || skill_table[sn]->skill_level[dch->class] == 0)
                continue;
            if (dch->exp >= skill_table[sn]->skill_level[dch->class] || value == 0)
            {
    ...
    ch_printf(dch, "%s and %s have been fused together.\n\r", dch->name, sch->name);
}

/*
 * do_androidfuse: request and response (accept/deny) handling
 */
void do_androidfuse(CHAR_DATA *ch, char *argument)
{
    ...
    argument = one_argument(argument, arg);
        ...
        /* must be android (any subtype) to use this command */
        if (!(is_android(ch) || is_android_e(ch) || is_android_fm(ch) || is_android_h(ch)))
        {
            ch_printf(ch, "Huh?\n\r");
            return;
        }
    ...
    if (arg[0] == '\0')
    {
        ch_printf(ch, "Syntax: androidfuse <player name>|accept|deny\n\r");
        return;
    }

    /* ACCEPT: respondent agrees to fusion */
    if (!str_cmp(arg, "accept"))
    {
        if (!ch->pcdata->android_fuse_pending)
        {
            ch_printf(ch, "You have no pending fusion requests.\n\r");
            return;
        }
        /* find requester */
        {
            CHAR_DATA *rch = get_char_room(ch, ch->pcdata->android_fuse_requester);
            if (!rch)
            {
                ch_printf(ch, "%s is no longer here.\n\r", ch->pcdata->android_fuse_requester);
                STRFREE(ch->pcdata->android_fuse_requester);
                ch->pcdata->android_fuse_pending = FALSE;
                return;
            }
            android_fuse_execute(rch, ch);
            STRFREE(ch->pcdata->android_fuse_requester);
            ch->pcdata->android_fuse_pending = FALSE;
        }
        return;
    ...
    /* DENY: respondent refuses fusion */
    if (!str_cmp(arg, "deny"))
    {
        if (!ch->pcdata->android_fuse_pending)
        {
        ...
        }
        {
            CHAR_DATA *rch = get_char_room(ch, ch->pcdata->android_fuse_requester);
            if (rch)
                ch_printf(rch, "%s has denied your fusion request.\n\r", ch->name);
            ch_printf(ch, "You deny the fusion request from %s.\n\r", ch->pcdata->android_fuse_requester);
            STRFREE(ch->pcdata->android_fuse_requester);
            ch->pcdata->android_fuse_pending = FALSE;
        }
        return;
    ...
    /* INITIATE: request fusion with another player */
    if ((target = get_char_room(ch, arg)) == NULL)
    {
        ch_printf(ch, "There is nobody here named %s.\n\r", arg);
        return;
    }
    /* basic eligibility checks (level, NPC, existing fusion, android type, transformations) */
    if ( IS_NPC(target) || IS_NPC(ch) ||
        is_superandroid(ch) || is_superandroid(target) ||
        !is_android(ch) || !is_android(target) ||
        (is_android_fm(ch) && is_android_fm(target)) ||
        (is_android_e(ch) /* ... copy remaining checks from original ... */) )
    {
        ch_printf(ch, "You cannot fuse with %s.\n\r", target->name);
        return;
    }

    /* record pending request */
    target->pcdata->android_fuse_pending = TRUE;
    target->pcdata->android_fuse_requester = STRALLOC(ch->name);
    ch_printf(ch, "Fusion request sent to %s. Awaiting response.\n\r", target->name);
    ch_printf(target,
        "%s wants to fuse androids with you.\n"
        "Type 'androidfuse accept' to agree or 'androidfuse deny' to refuse.\n\r",
        ch->name);
}
...
void do_integrate(CHAR_DATA * ch, char *argument)
{
    ...
    if (wearing_sentient_chip(ch) && !is_android(ch) && !is_superandroid(ch) && !is_reploid(ch))
    {
        send_to_char("You can't use this while wearing a sentient chip.\n\r", ch);
        return;
    }
    ...
    if (!str_prefix(argument, "T4") &&
        (ro = wearing_t4(ch)) != NULL)
    {
        ch->pcdata->learned[gsn_t4] = 100;
        unequip_char(ch, ro);
        extract_obj(ro);
        send_to_char("Installation of T4 complete.\n\r", ch);
        return;
    }

    if (!str_prefix(argument, "T3") &&
        (ro = wearing_t3(ch)) != NULL)
    {
        ch->pcdata->learned[gsn_t3] = 100;
        unequip_char(ch, ro);
        extract_obj(ro);
        send_to_char("Installation of T3 complete.\n\r", ch);
        return;
    }

    if (!str_prefix(argument, "T2") &&
        (ro = wearing_t2(ch)) != NULL)
    {
        ch->pcdata->learned[gsn_t2] = 100;
        unequip_char(ch, ro);
        extract_obj(ro);
        send_to_char("Installation of T2 complete.\n\r", ch);
        return;
    }

    if (!str_prefix(argument, "T1") &&
        (ro = wearing_t1(ch)) != NULL)
    {
        ch->pcdata->learned[gsn_t1] = 100;
        unequip_char(ch, ro);
        extract_obj(ro);
        send_to_char("Installation of T1 complete.\n\r", ch);
        return;
    }

    if (!str_prefix(argument, "OMEGA") &&
        (ro = wearing_omega(ch)) != NULL)
    {
        ch->pcdata->learned[gsn_omega] = 100;
        unequip_char(ch, ro);
        extract_obj(ro);
        send_to_char("Installation of OMEGA complete.\n\r", ch);
        return;
    }

    if (!str_prefix(argument, "VM1") &&
        (ro = wearing_vm1(ch)) != NULL)
    {
        ch->pcdata->learned[gsn_vm1] = 100;
        unequip_char(ch, ro);
        extract_obj(ro);
        send_to_char("Installation of VM1 complete.\n\r", ch);
        return;
    }

    if (!str_prefix(argument, "VM2") &&
        (ro = wearing_vm2(ch)) != NULL)
    {
        ch->pcdata->learned[gsn_vm2] = 100;
        unequip_char(ch, ro);
        extract_obj(ro);
        send_to_char("Installation of VM2 complete.\n\r", ch);
        return;
    }

    if (!str_prefix(argument, "SUP") &&
        (ro = wearing_asuppress(ch)) != NULL)
    {
        ch->pcdata->learned[gsn_suppress] = 100;
        unequip_char(ch, ro);
        extract_obj(ro);
        send_to_char("Installation of SUP complete.\n\r", ch);
        return;
    }

    if (!str_prefix(argument, "SENTIENT") &&
        (ro = wearing_sentient_chip2(ch)) != NULL)
    {
        ch->pcdata->learned[gsn_sent_chip] = 100;
        unequip_char(ch, ro);
        extract_obj(ro);
        send_to_char("Installation of SENTIENT complete.\n\r", ch);
        return;
    }

    send_to_char("Invalid chip type.\n\r", ch);
}

OBJ_DATA *wearing_t4(CHAR_DATA *character)
{
  for (OBJ_DATA *object = character->first_carrying; object != NULL; object = object->next_content)
  {
    if (object->wear_loc == WEAR_SHIELD && object->pIndexData->vnum == 2303)
    {
      return object;
    }
  }
