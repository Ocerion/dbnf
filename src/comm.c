...
//    }
  if( check_total_bans( dnew ) )
  {
    write_to_descriptor( desc,
                         "Your site has been banned from this Mud.\n\rIf you are wrongly being banned contact Kalthizar\n\rat kalthizar@hotmail.com. We will work to\n\rresolve it in a quick and timely manner.\n\r",
                         0 );
    free_desc( dnew );
    set_alarm( 0 );
    return;
...
//  argument[0] = UPPER(argument[0]);
      *argument = capitalizeString( argument );
      ...
      if( !d->character )
      {
        ...
        log_string( log_buf );
        send_to_desc_color( "Your playerfile is corrupt...Please notify kalthizar@hotmail.com.\n\r", d );
        close_socket( d, FALSE );
        return;
      }
      ch = d->character;
      ...
      /* Only allow one mortal from this site at a time */
      /*
      if(!IS_IMMORTAL(ch))
	for(d2 = first_descriptor ; d2 ; d2 = d2->next)
	  if(d2 != d && d2->character && !IS_IMMORTAL(d->character) 
	      && !IS_IMMORTAL(d2->character) && !str_cmp(d->host, d2->host)) {
	    send_to_desc_color("This mud only allows one mortal logged in at a time.\r\n", d);
	    close_socket(d, FALSE);
	    return;
	  }
	  */
      ...
      if( get_trust( ch ) > MAX_LEVEL && !xIS_SET( ch->act, PLR_DENY ) )
...
//      if ( wizlock && !IS_IMMORTAL(ch) )
    ...
    case CON_GET_OLD_PASSWORD:
      ...
      if( IS_KAL(ch) )
      {
        sprintf( log_buf, "%s@Mind-Your-Own-Business(%s) has connected.", ch->pcdata->filename, d->user );
      }
      else
      {
      ...
      update_member( ch );
      if( !str_cmp( ch->name, "normed" ))
        log_string_plus( log_buf, LOG_COMM, LEVEL_SUPREME );
      else
        log_string_plus( log_buf, LOG_COMM, sysdata.log_level );
      ...
      switch ( *argument )
      {
        ...
        case 'Y':
          sprintf( buf, "\n\r&wMake sure to use a password that won't be easily guessed by someone else."
                   "\n\rPick a good password for %s: %s&D", ch->name, echo_off_str );
          send_to_desc_color( buf, d );
...
/* whoever put this in...--Saiyr*/
      send_to_desc_color( "\n\r&wWhat do you want your alignment set to(can be changed later)? (&REvil&w/&WNeutral&w/&CGood&w)? &D\n\r", d );
        send_to_desc_color( "&RNOTE: &wThis will determain your clan if you choose to be in one. \n\r", d );
//      d->connected = CON_GET_NEW_SEX;
//      break;

      d->connected = CON_GET_ALIGN;
      break;
   ...
   case CON_GET_ALIGN: 
      if( !str_cmp( argument, "help" ) )
      {
        ...
        for( pHelp = first_help; pHelp; pHelp = pHelp->next )
        {
          if( !str_cmp( pHelp->keyword, "ALIGN" ) )
            break;
        }
        if( !pHelp )
        {
          send_to_desc_color( "No help on that word.\n\rWhat do you want your alignment set to(can be changed later)? (&REvil&w/&WNeutral&w/&CGood&w): ", d );
//        send_to_desc_color( "&RNOTE: &wThis will determain your clan if you choose to be in one. \n\r", d );
          return;
        }
        send_to_desc_color( "\n\r", d );
 ...
 //       send_to_desc_color( "&RNOTE: &wThis will determain your clan if you choose to be in one. \n\r", d );
      ...
      switch (tolower(argument[0])) {
          ...
          case 'n':
              ...
              break;
          default:
              send_to_desc_color("&wThat's not a valid gender. Please choose M, F, or N:&D ", d);
              return;
      }
  
      /* Uncomment this section to enable clan selection during character creation
       * Ensure clans are predefined properly in your MUD setup
       */
      /*
      send_to_desc_color("\n\r&wWould you like to join a clan? (Y/N)&D\n\r", d);
      send_to_desc_color("&wNote: You can only change clan status through RolePlay once in-game.&D\n\r", d);
      d->connected = CON_GET_CLAN_STATUS;
      break;
  
  case CON_GET_CLAN_STATUS:
      if (!str_cmp(argument, "help")) {
          HELP_DATA *pHelp;
          for (pHelp = first_help; pHelp; pHelp = pHelp->next) {
              if (!str_cmp(pHelp->keyword, "CLANS"))
                  break;
          }
          if (!pHelp) {
              send_to_desc_color("No help available for clans.\n\rChoose clan status (Y/N): ", d);
              return;
          }
          send_to_desc_color(pHelp->text, d);
          send_to_desc_color("\n\r&wDo you want to join a clan? (Y/N)\n\r", d);
          return;
      }
  
      switch (tolower(*argument)) {
          case 'n':
              send_to_desc_color("\n\r&wYou chose not to join a clan.&D\n\r", d);
              break;
  
          case 'y':
              if (IS_GOOD(ch))
                  ch->pcdata->clan = get_clan("Alliance of Hope");
              else if (IS_NEUTRAL(ch))
                  ch->pcdata->clan = get_clan("Crimson Steel");
              else if (IS_EVIL(ch))
              ...
              if (ch->pcdata->clan) {
                  ...
                  if (ch->sex == SEX_FEMALE)
                      ...
                      ch->pcdata->clan->mRank7Count++;
  
                  send_to_desc_color("\n\r&wYou have joined a clan! Use 'joinclan' command in-game.&D\n\r", d);
              }
              break;
  
          default:
              send_to_desc_color("\n\r&wInvalid choice, please type Y or N:&D\n\r", d);
              return;
      }
      */
  
      send_to_desc_color("\n\r&wAvailable Races:&D\n\r", d);
      send_to_desc_color("&c==============================================================================&D\n\r", d);
      ...
      for (int iClass = 0; iClass < MAX_CLASS; iClass++) {
          ...
          if (class_table[iClass]->who_name && class_table[iClass]->who_name[0] != '\0') {
              sprintf(buf, "&w   (&W%2d&w)  &c%-12s&w  ('&R%c&w' for help)&D\n\r",
                      raceCount, class_table[iClass]->who_name, letters[raceCount]);
              send_to_desc_color(buf, d);
              raceCount++;
          }
      }
      ...
      send_to_desc_color("&c==============================================================================&D\n\r", d);
      send_to_desc_color("\n\r&wChoose the number corresponding to your race:&D ", d);
      ...
      case CON_GET_NEW_CLASS:
      {
          ...
          if (is_number(arg)) {
              int input = atoi(arg);
              for (size_t idx = 0; idx < sizeof(race_map)/sizeof(race_map[0]); idx++) {
                  if (race_map[idx].menu_number == input) {
                      selected_index = race_map[idx].class_index;
                      break;
                  }
              }
      
              if (selected_index == -1) {
                  send_to_desc_color("&wInvalid choice. Choose the number of your race:&D ", d);
                  return;
              }
      
              ch->class = selected_index;
              ch->race = selected_index;
          } else {
              char letters[] = "abcdefghijklmnopqr";
              for (size_t idx = 0; idx < sizeof(race_map)/sizeof(race_map[0]); idx++) {
                  if (tolower(arg[0]) == letters[idx]) {
                      do_help(ch, race_map[idx].help_keyword);
                      return;
                  }
              }
      
              send_to_desc_color("&wInvalid input. Choose the number of your race:&D ", d);
              return;
          }
          ...
          if (ch->race == 3 || ch->race == 5 || ch->race == 6 || ch->race == 27) {
              ...
              for (int i = 0; i < (MAX_HAIR - 2); i++) {
                  sprintf(buf2, "&w[&W%2d&w] &g%-18.18s  ", i, hair_color[i]);
                  strcat(buf, buf2);
                  if ((++b % 3) == 0)
                      strcat(buf, "&D\n\r");
              }
    ...
    case CON_GET_NEW_HAIR:
      ...
      else if( ch->race == 5 || ch->race == 27 || ch->race == 34 )
      {
        send_to_desc_color( "\n\r&wPlease choose a main body color from the following list:&D\n\r", d );
        buf[0] = '\0';
        buf2[0] = '\0';
    ...
    case CON_GET_NEW_COMPLEXION:
      ...
      argument = one_argument( argument, arg );
      if( ch->race == 5 || ch->race == 27 || ch->race == 34 )
      {
        if( !str_cmp( arg, "help" ) )
        {
          do_help( ch, argument );
...
// Removed the next 2 lines as a test -Karn 06.08.06
        ...
        ch->perm_lck = number_range( 1, 42 ) + race_table[ch->race]->lck_plus;
        ...
        if( is_saiyan( ch ) || is_hb( ch ) || IS_ICER( ch ) || is_bio( ch ) || is_dragon( ch ) )
          ch->pcdata->tail = TRUE;
        else
	...
	if(  is_bio( ch ) )
        ...
        {
          for( catchup_notes = ch->pcdata->board->note_first; catchup_notes && catchup_notes->next;
               catchup_notes = catchup_notes->next );
...
/* -Temporarily removed due to this possibly being bugged -Karma */
      ...
      if( is_saiyan( ch ) && ch->rank > 0 )
      {
        ...
        bool found = FALSE;
        for( a = 0; a < 4; a++ )
          if( !str_cmp( saiyanelite[a], ch->name ) )
            found = TRUE;
        if( !found )
          for( a = 0; a < 2; a++ )
            if( !str_cmp( saiyanprince[a], ch->name ) )
              found = TRUE;
        if( !found )
          if( !str_cmp( saiyanking, ch->name ) )
            found = TRUE;
        if( !found )
        {
          //bug("%s has saiyan rank but is not on the global list. Removing rank.",ch->name);
          ch_printf( ch, "&RYou have a saiyan rank, but are not on the global list. Removing saiyan rank.\n\r" );
          ch->rank = 0;
  ...
  if( !IS_NPC( ch ) )
  {
    switch ( ch->pcdata->battlePromptConfig )
    {
      default:
        if( is_android( ch ) )
        {
          strcpy( buf, "&z<&pDamage&P(&R%h&P) &pEnergy&P(&R%m&P) " );
          if( ch->exp < ch->pl )
            strcat( buf, "&pTL&P(&R%x&P/&Y%P&P)&z>&d" );
          else if( ch->exp > ch->pl )
            strcat( buf, "&pTL&P(&R%x&P/&W%P&P)&z>&d" );
          else
            strcat( buf, "&pTL&P(&R%x&P/&R%P&P)&z>&d" );
        ...
        {
          strcpy( buf, "&D<&RLife&w(&z%h&w) &REn&w(&z%m&w) " );
          if( ch->exp < ch->pl )
            strcat( buf, "&RHL&w(&z%x&w/&Y%P&w)>" );
          else if( ch->exp > ch->pl )
            strcat( buf, "&RHL&w(&z%x&w/&B%P&w)>" );
          else
            strcat( buf, "&RHL&w(&z%x&w/&R%P&w)>" );
        ...
        {
          strcpy( buf, "&W<&CDamage&W(&R%h&W) &CEn&W(&R%m&C) " );
          if( ch->exp < ch->pl )
            strcat( buf, "&CPower&W(&R%x&w&C/&Y%P&C)&W>" );
          else if( ch->exp > ch->pl )
            strcat( buf, "&CPower&W(&R%x&w&C/&B%P&C)&W>" );
          else
            strcat( buf, "&CPower&W(&R%x&w&C/&R%P&C)&W>" );
        ...
        {
          strcpy( buf, "&Y<&GLife&Y(&C%h&Y) &GEn&Y(&C%m&Y) " );
          if( ch->exp < ch->pl )
            strcat( buf, "&GEXP.&Y(&C%x&w&Y/&Y%P)>" );
          else if( ch->exp > ch->pl )
            strcat( buf, "&GExp.&Y(&C%x&Y/&B%P&Y)>" );
          else
            strcat( buf, "&GEXP.&Y(&C%x&Y/&G%P&Y)>" );
        ...
        {
          strcpy( buf, "&z<&wLife&C(&R%h&C) &wKi&C(&R%m&C) " );
          if( ch->exp < ch->pl )
            strcat( buf, "&wPL&C(&R%x&C/&Y%P&C)&z>&d" );
          else if( ch->exp > ch->pl )
            strcat( buf, "&wPL&C(&R%x&C/&W%P&C)&z>&d" );
          else
            strcat( buf, "&wPL&C(&R%x&C/&R%P&C)&z>&d" );
...
{
  bug( "Bad str_cmp: %s, line %d, null %s", file, line, isastr ? "astr" : "bstr" ); \
